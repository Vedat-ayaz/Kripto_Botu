...
class TrendFollowingStrategy:
    ...
    def __init__(
        ...
        # EMA Stack (Golden Cross) — EMA50 > EMA200 zorunlu mu?
        # True: sadece orta-uzun vade hizalı (EMA50 > EMA200) girişe izin ver.
        # Bounce-in-downtrend: fiyat EMA200'ü geçebilir ama EMA50 hâlâ aşağıda → BLOK.
        ema_stack_required: bool = False,
        # Orta-vadeli momentum filtresi — 14 günlük düşüş tuzağını eler
        # 15m'de 1344 bar = 14 gün. Coin bu sürede -%pct'ten fazla düştüyse LONG engelle.
        # Varsayılan kapalı (0). Aktif olduğunda: close < price_Nd_ago * pct → HOLD
        medium_momentum_bars: int = 0,       # 0=devre dışı; 1344=14 gün @ 15m
        medium_momentum_pct: float = 0.90,   # 14 günde -%10 düşüş → LONG blok
        ...
        # should_exit ATR tampon ölçekleri — TF-aware kalibrasyon için
        # 1h ATR14 ≈ 2× 15m ATR56 → buffer mesafeleri % olarak eşdeğer olsun:
        # 1h varsayılanlar: short=1.5, long_up=3.5, long_dn=2.5, ema_up=4.5, ema_dn=3.0
        # 15m (ATR 2× küçük): × 2.0 → short=3.0, long_up=7.0, long_dn=5.0, ema_up=9.0, ema_dn=6.0
        se_short_buf_mult: float = 1.5,     # SHORT'ta EMA50 bounce buffer (1h=1.5, 15m=3.0)
        se_long_buf_up: float = 3.5,        # LONG uptrend should_exit buffer (1h=3.5, 15m=7.0)
        se_long_buf_dn: float = 2.5,        # LONG downtrend should_exit buffer (1h=2.5, 15m=5.0)
        se_ema_exit_up: float = 4.5,        # LONG EMA exit buffer uptrend (1h=4.5, 15m=9.0)
        se_ema_exit_dn: float = 3.0,        # LONG EMA exit buffer downtrend (1h=3.0, 15m=6.0)
        ...
        self.ema_stack_required        = ema_stack_required
        self.medium_momentum_bars      = medium_momentum_bars
        self.medium_momentum_pct       = medium_momentum_pct
        ...
        # should_exit tampon çarpanları (TF-aware kalibrasyon)
        self._se_short_buf_mult = se_short_buf_mult
        self._se_long_buf_up    = se_long_buf_up
        self._se_long_buf_dn    = se_long_buf_dn
        self._se_ema_exit_up    = se_ema_exit_up
        self._se_ema_exit_dn    = se_ema_exit_dn
        self.indicators            = indicators or TechnicalIndicators()
        self._last_logged_bar: dict[str, str] = {}
        # Yıllık testlerde trend-long kalite ağırlığı iyi rebound girişlerini kaçırdı.
        # Range-long ve short kalite ise stabil/ayı dönemindeki kayıpları azalttı.
        _quality_weight_raw = os.environ.get("M1_QUALITY_WEIGHT")
        if _quality_weight_raw is None:
            _quality_trend_default = 0.0
            _quality_range_default = 1.0
            _quality_short_default = 1.0
        else:
            _quality_weight_scale = _env_float("M1_QUALITY_WEIGHT", 0.0, 0.0, 1.0)
            _quality_trend_default = _quality_weight_scale
            _quality_range_default = _quality_weight_scale
            _quality_short_default = _quality_weight_scale
        self._quality_weight_trend_scale = _env_float(
            "M1_QUALITY_TREND_WEIGHT", _quality_trend_default, 0.0, 1.0,
        )
        self._quality_weight_range_scale = _env_float(
            "M1_QUALITY_RANGE_WEIGHT", _quality_range_default, 0.0, 1.0,
        )
        self._quality_weight_short_scale = _env_float(
            "M1_QUALITY_SHORT_WEIGHT", _quality_short_default, 0.0, 1.0,
        )
        ...
        # M1 default: sinyal varsa aynı coin tekrar işlem alabilsin. Eski pause
        # davranışı istenirse M1_COIN_PAUSE=1 ile açılabilir.
        self._coin_pause_enabled = bool(int(os.environ.get("M1_COIN_PAUSE", "0")))
    ...
    def _blend_quality_weight(
        self, base: dict[str, float], quality_weight: float, quality_scale: float,
    ) -> dict[str, float]:
        q_weight = quality_weight * quality_scale
        if q_weight <= 0:
            return base
        weights = {key: value * (1.0 - q_weight) for key, value in base.items()}
        weights["quality"] = q_weight
        return weights
    ...
    def generate_signal(self, symbol: str, df: pd.DataFrame,
        ...
        trend_q_up = row.get("trend_quality_up", 0.5)
        trend_q_dn = row.get("trend_quality_down", 0.5)
        ...
        if allow_short and close < ema_slow * self.short_ema_pct:   # SHORT tetik eşiği (1m: 0.9995, 1h: 0.985)
            ...
            if rsi >= 32 and _slope_ok and _short_mom_ok:
                _short_score, _short_detail = self._composite_score_short(
                    ...
                    obv, obv_ema, tsmom, is_trending, trend_q_dn,
                )
        ...
        # ── EMA Stack filtresi (Golden Alignment) ─────────────────────
        # EMA50 < EMA200: orta vadeli trend hâlâ aşağı → bounce tuzağı riski.
        # Gerçek trend dönüşü için EMA50'nin EMA200'ü geçmesi (golden cross) beklenir.
        # Bounce-in-downtrend pattern'i: fiyat EMA200'ü aşar ama EMA50 aşağıda kalır.
        if self.ema_stack_required and ema_fast < ema_slow:
            return self._hold(
                symbol, df,
                f"EMA Stack: EMA50={ema_fast:.4f} < EMA200={ema_slow:.4f} — golden cross yok, bounce tuzağı"
            )
        ...
        # ── Orta-Vadeli Momentum Filtresi (14 günlük bear bounce engeli) ──
        # Coin N bar öncesine göre pct'ten fazla düştüyse LONG engelle.
        # Büyük cap'ların (ETH/SOL/BNB) bear dönemindeki bounce-then-continue-down
        # pattern'ini filtreler. 15m'de medium_momentum_bars=1344 = 14 gün.
        if self.medium_momentum_bars > 0 and len(df) >= self.medium_momentum_bars:
            price_14d_ago = df["close"].iloc[-self.medium_momentum_bars]
            if not pd.isna(price_14d_ago) and price_14d_ago > 0:
                momentum_14d = close / price_14d_ago
                if momentum_14d < self.medium_momentum_pct:
                    return self._hold(
                        symbol, df,
                        f"14g momentum negatif: {close:.4f}/{price_14d_ago:.4f}={momentum_14d:.3f} "
                        f"< {self.medium_momentum_pct} — bear bounce tuzağı"
                    )
        ...
        score, detail = self._composite_score(
            close, ema_fast, ema_slow, rsi, adx, macd, macd_hist,
            bb_pct_b, bb_width, stoch_k, volume, volume_sma,
            obv, obv_ema, tsmom, is_trending, trend_q_up,
        )
        ...
        # EMA200 slope boost — devre dışı (6M testinde iyi girişleri de engelliyor)
        # TODO: sadece STRONG_BEAR'da veya daha dar EMA200 slope eşiğiyle tekrar test et
    ...
    def _composite_score(
        ...
        is_trending: bool, trend_quality: float = 0.5,
    ) -> tuple[float, str]:
        """
        ...
        # ── 9. Trend kalitesi — ADX/Hurst/ER/CI/TSMOM birleşimi ───────
        try:
            tq = float(trend_quality)
        except (TypeError, ValueError):
            tq = 0.5
        components["quality"] = float(np.clip(tq, 0.0, 1.0)) if not pd.isna(tq) else 0.5

        # ── Ağırlıklar (trending vs ranging) ──────────────────────────
        # Varsayılan: trend-long eski ağırlıkta kalır; range-long ve short
        # kalite skoru alır. M1_QUALITY_WEIGHT toplu override için kullanılabilir.
        if is_trending:
            base_weights = {
            ...
            weights = self._blend_quality_weight(
                base_weights, 0.12, self._quality_weight_trend_scale,
            )
        else:
            base_weights = {
            ...
            weights = self._blend_quality_weight(
                base_weights, 0.07, self._quality_weight_range_scale,
            )
    ...
    def _composite_score_short(
        ...
        weights = self._blend_quality_weight(
            base_weights, 0.12, self._quality_weight_short_scale,
        )
    ...
    def should_exit(self, symbol: str, df: pd.DataFrame, entry_price: float,
        ...
        adx_val = float(row.get("adx", 20.0) or 20.0)
        ...
        # Güçlü uptrend: ADX > 35 + coin uptrend → INJ/NEAR gibi pump'larda erken çıkışı önle
        _strong_uptrend = _coin_in_uptrend and adx_val >= 35

        # ── Minimum kâr kapısı — erken SE engelle ────────────────────────
        # entry_price > 0 geldiyse: pozisyon %0–0.5 kârdayken RSI/BB/regime SE'yi blokla.
        # Neden: INJ +1.1%'de SE çıktı (coin sonra +90% yaptı). Küçük indicator salınımı
        # yeterli kâr olmadan SE tetiklememeli. Zarar bölgesinde (<0) SE normale çalışır.
        _pos_pnl_pct = 0.0
        if entry_price > 0:
            if is_short:
                _pos_pnl_pct = (entry_price - close) / entry_price
            else:
                _pos_pnl_pct = (close - entry_price) / entry_price
        if _coin_in_uptrend:
            # v26: 2.5×ATR → 3.5×ATR → TF-aware (15m: 7.0×ATR)
            long_buffer = self._se_long_buf_up * atr if atr > 0 else ema_fast * (self._se_long_buf_up * 0.01)
        else:
            # v26: 1.5×ATR → 2.5×ATR → TF-aware (15m: 5.0×ATR)
            long_buffer = self._se_long_buf_dn * atr if atr > 0 else ema_fast * (self._se_long_buf_dn * 0.01)
        # SHORT için tampon: TF-aware (1h=1.5×ATR=0.78%, 15m=3.0×ATR=1.57%)
        short_buffer = self._se_short_buf_mult * atr if atr > 0 else ema_fast * (self._se_short_buf_mult * 0.01)
        ...
        if is_short:
            ...
            # v37: sadece fiyat EMA50'nin ALTINDAYSA kapat — üstündeyse bearish trend devam ediyor
            if regime < 0.4 and bb_pct_b < 0.05 and close > ema_fast * 0.98:
                return True, f"SHORT kapama: Ranging BB alt bandı + fiyat EMA yakınında (%B={bb_pct_b:.2f})"
            # 4. Piyasa güçlü YUKARI trending'e döndü (SHORT tehlikeli)
            # v37 FIX: Güçlü AŞAĞI trend de regime>0.75 verir → SHORT çıkarmaz!
            # Sadece fiyat EMA50'nin ÜSTÜNDEYSe + güçlü trending → gerçek YUKARI trend dönüşü
            if regime > 0.75 and close > ema_fast + self._se_short_buf_mult * atr:
                return True, f"SHORT kapama: Yukari trending (regime={regime:.2f}, close > EMA50+{self._se_short_buf_mult:.0f}×ATR)"
            return False, ""

        # ── LONG çıkış koşulları ──────────────────────────────────────────
        # FIX #2: SE erken çıkış eşikleri sıkılaştırıldı
        # Analiz: 4-12 saatlik erken SE çıkışları -$658 kaybettirdi (%28 WR)
        # Trending coinler (NEAR +65%, INJ +22%) için bot çok erken çıkıyordu
        # Çözüm: EMA50 tamponu büyütüldü, regime eşiği düşürüldü

        # EMA50 kırılımı — TF-aware tampon (1h=4.5/3.0×ATR, 15m=9.0/6.0×ATR)
        if _coin_in_uptrend:
            ema_exit_buffer = self._se_ema_exit_up * atr if atr > 0 else ema_fast * (self._se_ema_exit_up * 0.01)
        else:
            ema_exit_buffer = self._se_ema_exit_dn * atr if atr > 0 else ema_fast * (self._se_ema_exit_dn * 0.01)
        if close < ema_fast - ema_exit_buffer:
            _buf_x = "4.5" if _coin_in_uptrend else "3.0"
            return True, f"EMA50 belirgin kırıldı: close={close:.4f} < EMA50-{_buf_x}ATR={ema_fast - ema_exit_buffer:.4f}"

        # RSI aşırı alım + MACD negatife döndü
        # v38: ADX≥35 + uptrend → eşik 90 | v39: uptrend → 85 | diğer: 82
        _rsi_thr = 90 if _strong_uptrend else (85 if _coin_in_uptrend else 82)
        if rsi > _rsi_thr and macd_hist < 0:
            if 0.0 < _pos_pnl_pct < 0.015:  # %1.5 kâr eşiği altında → erken SE blok
                pass  # Yeterli kâr yok, RSI/MACD salınımı SE'yi tetiklemesin
            elif _strong_uptrend and 0.015 <= _pos_pnl_pct < 0.15:
                # v40: Güçlü uptrend (ADX≥35) + kâr %1.5-15 arası → RSI dip çıkışı blokla
                # Neden: INJ ADX≥35 iken %16 kârda RSI>90 + kısa MACD cross → erken SE
                #        coin sonra %60+ daha yükseldikçe trailing stop pozisyonu korur
                pass
            else:
                return True, f"RSI aşırı alım ({rsi:.1f}>{_rsi_thr}) + MACD negatif"

        # Ranging piyasada BB üst bandına ulaşıldı
        # v38: Güçlü trendde (ADX≥35) BB exit çalışmaz — trend devam ediyor
        if regime < 0.30 and bb_pct_b > 0.97 and not _strong_uptrend:
            return True, f"Ranging piyasa BB üst bandı aşıldı (%B={bb_pct_b:.2f})"

        # Rejim tamamen ranging'e döndü
        # v38: Güçlü trendde bu çıkış da çalışmaz — INJ gibi pump'ta regime geçici dalar
        if regime < 0.18 and not _strong_uptrend:
    ...
    def apply_short_regime(self, is_bear: bool) -> None:
        """
        v35: Rejime göre SHORT tetik eşiğini ayarla.
        BEAR'da: 0.998 (gevşek — ETH/BNB/SOL yavaş düşüşlerde SHORT alabilsin)
        NEUTRAL/BULL'da: 0.985 (katı — bull recovery'de yanlış SHORT önlenir)
        """
        self.short_ema_pct = 0.998 if is_bear else 0.985
    ...
    def record_outcome(self, symbol: str, won: bool, pnl: float = 0.0) -> None:
        ...
        if not self._coin_pause_enabled:
            return
