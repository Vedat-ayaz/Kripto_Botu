...
def _efficiency_ratio(close: pd.Series, period: int = 20) -> pd.Series:
    ...
    1.0'a yakın değerler fiyatın aynı yöne temiz ilerlediğini, 0.0'a yakın
    değerler ise aynı mesafeyi zikzakla aldığını gösterir.
    """
...
class TechnicalIndicators:
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ── Trend Kalitesi (M1 v41) ───────────────────────────────────
        # Sabit coin threshold'u yerine, o anki trendin temizliğini ölçen
        # ortak bir skor. LONG ve SHORT yönleri ayrı tutulur; ikisi de aynı
        # "common trend quality" zeminini kullanır.
        out["efficiency_ratio"] = _efficiency_ratio(out["close"], self.tsmom_period)

        ci_score = ((61.8 - out["choppiness"]) / 23.6).clip(0, 1).fillna(0.5)
        er_score = out["efficiency_ratio"].fillna(0.5).clip(0, 1)
        adx_q = ((out["adx"] - 15) / 25).clip(0, 1).fillna(0.0)
        hurst_q = ((out["hurst"] - 0.45) / 0.20).clip(0, 1).fillna(0.5)
        tsmom_pos = (out["tsmom"].clip(lower=0, upper=2.0) / 2.0).fillna(0.0)
        tsmom_neg = ((-out["tsmom"]).clip(lower=0, upper=2.0) / 2.0).fillna(0.0)

        ema_gap = ((out["close"] - out[self.ema_slow_col]) / out[self.ema_slow_col].replace(0, np.nan)).fillna(0.0)
        ema_fast_gap = ((out[self.ema_fast_col] - out[self.ema_slow_col]) / out[self.ema_slow_col].replace(0, np.nan)).fillna(0.0)
        up_bias = (0.55 * (ema_gap * 8).clip(0, 1) + 0.45 * (ema_fast_gap * 10).clip(0, 1)).clip(0, 1)
        down_bias = (0.55 * ((-ema_gap) * 8).clip(0, 1) + 0.45 * ((-ema_fast_gap) * 10).clip(0, 1)).clip(0, 1)
        ...
        common_q = (
            ...
            + er_score * 0.22
            + ci_score * 0.18
            + out["regime_score"].fillna(0.5) * 0.12
        ).clip(0, 1)
        out["trend_quality_up"] = (common_q * (0.55 + 0.30 * up_bias + 0.15 * tsmom_pos)).clip(0, 1)
        out["trend_quality_down"] = (common_q * (0.55 + 0.30 * down_bias + 0.15 * tsmom_neg)).clip(0, 1)
