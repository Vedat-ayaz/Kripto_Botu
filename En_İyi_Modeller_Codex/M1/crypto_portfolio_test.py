...
from strategy.adaptive_coin_profile import AdaptiveCoinProfile
...
UNIVERSE = [
    ...
    # KALDIRILDI: AVAX (0% WR 6 trade 1M), TRX (erratic, stop-hunt), OP (choppy düşük hacim)
    # KALDIRILDI: LINK (6M %17 WR -$225, yapısal uyumsuz), LTC (6M %36 WR -$70, choppy)
    "ADA/USDT", "DOGE/USDT",
    "DOT/USDT", "ATOM/USDT",
    # Büyüyen ekosistem
    # KALDIRILDI: FET (yapısal uyumsuz, 1M'de +62% yaptı bot -14$ yaptı)
    "NEAR/USDT", "UNI/USDT", "APT/USDT", "INJ/USDT",
    "ARB/USDT",
    # Özel / sabit
    # KALDIRILDI: ETC (6M %33 WR -$203, choppy), HBAR (%14 WR -$36), VET (%38 WR -$71, düşük hacim)
    "LEO/USDT", "ALGO/USDT", "FIL/USDT",
    # M6 v7: yeni yüksek-momentumlu coinler (2024-2025 büyük hareketler)
    # KALDIRILDI: TIA (6M %38 WR -$109)
    "SUI/USDT", "TON/USDT", "JUP/USDT", "WIF/USDT",
]

# Varsayılan aktif coin listesi — universe modunda "her zaman dahil" çekirdek coinler
# KALDIRILDI: AVAX (%12 WR), TRX (%14 WR), LINK (%17 WR), ETC (%33 WR)
SYMBOLS = [
    "BNB/USDT", "ETH/USDT", "SOL/USDT", "XRP/USDT",
...
# ── SHORT-Only Universe ──────────────────────────────────────────────────────
# Bu coinler LONG işlemde kötü performans gösteriyor (düşük WR, stop-hunt).
# Ama BEAR rejiminde SHORT için güçlü adaylar (M5 canlı veride kanıtlandı).
# Kural: Bu coinlerde SADECE SHORT açılır, LONG sinyali bloklanır.
SHORT_ONLY_UNIVERSE = [
    # HBAR KALDIRILDI: stop-hunt çok agresif (6h SL 2x → -$28 ayı haftasında)
    # ETC: M5'te %3.48 SHORT kârı (+$3.16), LONG choppy, SHORT daha stabil
    "ETC/USDT",
]

COIN_SELECT_N      = 26     # Tüm universe coinleri aktif (limit yok)
COIN_SELECT_MIN_SCORE = 0.20  # Daha düşük eşik — tüm universe değerlendirilsin
WFO_ENABLED    = False  # --wfo flag'i ile True yapılır
WFO_LOOKBACK   = 200    # WFO için önceki N gün

# Rejim → Universe modunda kullanılabilecek max coin sayısı
# Tüm 26 coin aktif — rejime göre sadece pozisyon boyutu küçülür (COIN_SELECT_N ile)
...
BASELINE = dict(
    ...
    ema_stack_required=False,   # v25: EMA50>EMA200 golden cross zorunluluğu (bounce-in-downtrend engeli)
    medium_momentum_bars=0,     # v26: 0=kapalı; 1344=14 gün @ 15m (bear bounce engeli)
    medium_momentum_pct=0.90,   # v26: 14 günde -%10 düşüş → LONG blok
    slope_bars=20,
...
# Per-coin overrides
# v26: trailing_stop_atr_multiplier tümü +2.0 artırıldı (54% SL → hedef <%40)
# Amaç: normal geri çekilmelerde stop yenmeden büyük trend tutulabilsin.
PROFILES: dict[str, dict] = {
    # v27 profil seviyelerine geri döndürüldü — regime boost fix ile bear churn engellenir
    # Asıl fix: bear'da coin_own_bull boost relaxation KALDIRILDI (v33 kodundaki değişiklik)
    "BNB/USDT":  dict(adx_threshold=33, rsi_lower=53, atr_stop_multiplier=2.5,  # 6M %30 WR -$40 → ADX 28→33, skor hafif sıkı
                      trailing_stop_atr_multiplier=9.0, entry_score_trend=0.74, entry_score_ranging=0.78,
                      max_position_pct=0.05, sl_cooldown_hours=72, breakout_bars=20),
    "ETH/USDT":  dict(adx_threshold=40, rsi_lower=54, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.0, entry_score_trend=0.78, entry_score_ranging=0.82,
                      max_position_pct=0.04, sl_cooldown_hours=96),  # 37 trade %35 WR → daha seçici
    "SOL/USDT":  dict(adx_threshold=35, rsi_lower=54, atr_stop_multiplier=2.2,
                      trailing_stop_atr_multiplier=11.5, entry_score_trend=0.73, entry_score_ranging=0.77,
                      max_position_pct=0.04, sl_cooldown_hours=60),  # trailing 9.5→11.5: 6M +$21 daha uzun tut
    "XRP/USDT":  dict(adx_threshold=26, rsi_lower=50, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=9.5, entry_score_trend=0.66, entry_score_ranging=0.70,
                      max_position_pct=0.08, sl_cooldown_hours=36),  # trailing 7.5→9.5: 6M +$28 daha uzun tut
    "ADA/USDT":  dict(adx_threshold=29, rsi_lower=52, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.5, entry_score_trend=0.72, entry_score_ranging=0.76,
                      max_position_pct=0.02, breakout_bars=24, sl_cooldown_hours=48),
    # v27 seviyesi
    "DOGE/USDT": dict(adx_threshold=32, rsi_lower=50, atr_stop_multiplier=3.0,
                      trailing_stop_atr_multiplier=8.5, entry_score_trend=0.73, entry_score_ranging=0.77,
                      max_position_pct=0.03, sl_cooldown_hours=48, breakout_bars=20),
    # AVAX UNIVERSE'DEN ÇIKARILDI — 6 trade 0% WR, stop-hunt pattern
    "AVAX/USDT": dict(adx_threshold=38, rsi_lower=55, atr_stop_multiplier=3.0,
                      trailing_stop_atr_multiplier=8.5, entry_score_trend=0.78, entry_score_ranging=0.82,
                      max_position_pct=0.01, sl_cooldown_hours=96),
    "DOT/USDT":  dict(adx_threshold=26, rsi_lower=50, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.5, entry_score_trend=0.66, entry_score_ranging=0.70,
                      max_position_pct=0.05, breakout_bars=24, sl_cooldown_hours=36),
    "LEO/USDT":  dict(adx_threshold=32, rsi_lower=52, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=9.0, entry_score_trend=0.74, entry_score_ranging=0.78,
                      risk_per_trade=0.006, max_position_pct=0.015, sl_cooldown_hours=72),
    # TRX UNIVERSE'DEN ÇIKARILDI — erratic/stop-hunt, her dönemde kayıp
    "TRX/USDT":  dict(adx_threshold=30, rsi_lower=52, atr_stop_multiplier=3.0,
                      trailing_stop_atr_multiplier=10.0, entry_score_trend=0.78, entry_score_ranging=0.82,
                      risk_per_trade=0.006, max_position_pct=0.01, breakout_bars=16,
                      sl_cooldown_hours=96, min_stop_pct=0.015),
    # ── Yeni coinler ──────────────────────────────────────────────────────────
    "LINK/USDT": dict(adx_threshold=25, rsi_lower=48, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.0, entry_score_trend=0.65, entry_score_ranging=0.70,
                      max_position_pct=0.08, sl_cooldown_hours=24),
    "LTC/USDT":  dict(adx_threshold=23, rsi_lower=48, atr_stop_multiplier=2.0,
                      trailing_stop_atr_multiplier=6.5, entry_score_trend=0.62, entry_score_ranging=0.67,
                      max_position_pct=0.08, sl_cooldown_hours=24),
    "ATOM/USDT": dict(adx_threshold=28, rsi_lower=50, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=12.0, entry_score_trend=0.68, entry_score_ranging=0.73,
                      max_position_pct=0.05, sl_cooldown_hours=48),  # trailing 9.5→12: 6M %52 WR -$12 → daha geniş trail
    "NEAR/USDT": dict(adx_threshold=33, rsi_lower=52, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=14.0, entry_score_trend=0.72, entry_score_ranging=0.76,
                      max_position_pct=0.04, sl_cooldown_hours=60),  # 6M %50 WR -$28 → trailing 11→14 (coin +71% yaptı, kaçırıldı)
    "UNI/USDT":  dict(adx_threshold=28, rsi_lower=50, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=9.0, entry_score_trend=0.68, entry_score_ranging=0.73,
                      max_position_pct=0.05, sl_cooldown_hours=36),  # trailing geniş → pozisyon daha uzun tutulur
    "APT/USDT":  dict(adx_threshold=36, rsi_lower=53, atr_stop_multiplier=3.2,
                      trailing_stop_atr_multiplier=12.0, entry_score_trend=0.76, entry_score_ranging=0.80,
                      max_position_pct=0.02, sl_cooldown_hours=96),  # 6M %28 WR -$102 → ADX 30→36, boyut 4→2%, cooldown 48→96h, trail geniş
    "INJ/USDT":  dict(adx_threshold=38, rsi_lower=54, atr_stop_multiplier=3.0,
                      trailing_stop_atr_multiplier=13.0, entry_score_trend=0.76, entry_score_ranging=0.80,
                      max_position_pct=0.03, sl_cooldown_hours=96),  # trailing 10→13: 6M +$10, 30% WR fat-tail → daha büyük trend yakala
    # FET UNIVERSE'DEN ÇIKARILDI — yapısal uyumsuz, 1M'de +62% yaptı bot -$14 yaptı
    "FET/USDT":  dict(adx_threshold=35, rsi_lower=55, atr_stop_multiplier=3.0,
                      trailing_stop_atr_multiplier=8.0, entry_score_trend=0.80, entry_score_ranging=0.84,
                      max_position_pct=0.01, sl_cooldown_hours=96),
    "ARB/USDT":  dict(adx_threshold=28, rsi_lower=50, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=12.5, entry_score_trend=0.68, entry_score_ranging=0.73,
                      max_position_pct=0.05, sl_cooldown_hours=48),  # trailing 10→12.5: 6M +$28 → daha uzun tut
    # OP UNIVERSE'DEN ÇIKARILDI — choppy, düşük hacim
    "OP/USDT":   dict(adx_threshold=32, rsi_lower=55, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.5, entry_score_trend=0.76, entry_score_ranging=0.80,
                      max_position_pct=0.01, sl_cooldown_hours=96),
    "ETC/USDT":  dict(adx_threshold=24, rsi_lower=48, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.0, entry_score_trend=0.64, entry_score_ranging=0.69,
                      max_position_pct=0.07, sl_cooldown_hours=24),
    "HBAR/USDT": dict(adx_threshold=26, rsi_lower=50, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.5, entry_score_trend=0.67, entry_score_ranging=0.72,
                      max_position_pct=0.05, sl_cooldown_hours=36),
    "ALGO/USDT": dict(adx_threshold=30, rsi_lower=51, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=9.5, entry_score_trend=0.70, entry_score_ranging=0.75,
                      max_position_pct=0.04, sl_cooldown_hours=60),  # %45 WR -$73 → daha seçici giriş, geniş trailing
    "VET/USDT":  dict(adx_threshold=25, rsi_lower=50, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=7.0, entry_score_trend=0.66, entry_score_ranging=0.71,
                      max_position_pct=0.05, sl_cooldown_hours=36),
    "FIL/USDT":  dict(adx_threshold=38, rsi_lower=54, atr_stop_multiplier=3.5,
                      trailing_stop_atr_multiplier=13.0, entry_score_trend=0.78, entry_score_ranging=0.82,
                      max_position_pct=0.02, sl_cooldown_hours=120),  # 6M %14 WR -$126 → ADX 30→38, boyut 4→2%, cooldown 60→120h, trail çok geniş
    # v27 seviyesine döndürüldü — regime fix ile bear churn durur
    "SUI/USDT":  dict(adx_threshold=30, rsi_lower=52, atr_stop_multiplier=2.8,
                      trailing_stop_atr_multiplier=8.0, entry_score_trend=0.72, entry_score_ranging=0.76,
                      max_position_pct=0.04, sl_cooldown_hours=48),
    "TIA/USDT":  dict(adx_threshold=28, rsi_lower=50, atr_stop_multiplier=3.0,
                      trailing_stop_atr_multiplier=8.0, entry_score_trend=0.68, entry_score_ranging=0.73,
                      max_position_pct=0.04, sl_cooldown_hours=36),
    "TON/USDT":  dict(adx_threshold=28, rsi_lower=52, atr_stop_multiplier=2.5,
                      trailing_stop_atr_multiplier=11.0, entry_score_trend=0.70, entry_score_ranging=0.75,
                      max_position_pct=0.08,          # BULL'da tam boyut (star performer kârı koru)
                      bear_max_position_pct=0.03,     # BEAR'da maks %3 — volatil LONG kaybını sınırla
                      sl_cooldown_hours=48),
    "JUP/USDT":  dict(adx_threshold=30, rsi_lower=51, atr_stop_multiplier=3.0,
                      trailing_stop_atr_multiplier=10.0, entry_score_trend=0.70, entry_score_ranging=0.75,
                      max_position_pct=0.04, sl_cooldown_hours=60),  # %41 WR -$52 → daha seçici, trailing geniş
    "WIF/USDT":  dict(adx_threshold=34, rsi_lower=52, atr_stop_multiplier=3.5,
                      trailing_stop_atr_multiplier=13.0, entry_score_trend=0.74, entry_score_ranging=0.78,
                      max_position_pct=0.03, sl_cooldown_hours=72),  # 6M %50 WR ama -$34 → ADX 30→34, trail 11→13, cooldown 48→72h
}
...
WARMUP_BARS = 210   # EMA200 + buffer (1h modu)

# ── TF-aware warmup bar sayıları ─────────────────────────────────────────────
# 15m kalibrasyonunda EMA periyotları 4× ölçeklendi (ema_slow=800) → 10 günlük warmup gerekli.
# 960 bar × 15min = 14400min = 10 gün (EMA800 için min ~8.4 gün).
_TF_WARMUP_BARS: dict[str, int] = {
    "1m":  300,   # 300×1min  =  5h  (EMA200×1m için yeterli)
    "5m":  300,   # 300×5min  = 25h  (EMA200×5m için yeterli)
    "15m": 960,   # 960×15min = 10g  (EMA800×15m için ≥8.4 gün)
    "1h":  210,   # 210×60min = 8.75g (EMA200×1h için yeterli)
}
...
# ── M5 Signal Pyramid ────────────────────────────────────────────────────────
# Açık LONG pozisyona yeni BUY sinyali gelince ek lot ekle.
# Akademik temel: Dennis/Eckhardt Turtle Rules (1983) + Elder Triple Screen.
# M5_SIG_PYR=1 → kâr kapılı (+%0.8), M5_SIG_PYR=2 → her BUY sinyalinde
M5_SIG_PYR        = int(os.environ.get("M5_SIG_PYR",         "0"))
M5_SIG_PYR_MAX    = int(os.environ.get("M5_SIG_PYR_MAX",     "2"))
M5_SIG_PYR_SIZE   = float(os.environ.get("M5_SIG_PYR_SIZE",  "0.75"))
M5_SIG_PYR_BARS   = int(os.environ.get("M5_SIG_PYR_BARS",    "4"))
M5_SIG_PYR_PROFIT = float(os.environ.get("M5_SIG_PYR_PROFIT","0.008"))
# V3: Rejim kapısı — 1=sadece coin kendi EMA200 üstündeyken ekle (coin_own_bull)
#     bear'de averajlama riskini ortadan kaldırır, bull'da tam güç
M5_SIG_PYR_BULL_ONLY = int(os.environ.get("M5_SIG_PYR_BULL_ONLY", "0"))  # 0=kapalı, 1=coin_own_bull şartı

# ── SHORT PYRAMID (Kademeli Short) ───────────────────────────────────────────
# LONG piramit gibi: fiyat düştükçe short pozisyona ek lot ekle.
# İki mekanizma:
#   1. Kâr-bazlı (Turtle): SHORT %THRESH1 kârda → lot ekle, %THRESH2'de bir daha
#   2. Sinyal-bazlı: açık SHORT'a yeni SHORT sinyali gelince ek lot ekle
# SHORT_PYR=1 → kâr kapılı sinyal piramit (varsayılan AÇIK)
# SHORT_PYR=0 → devre dışı
SHORT_PYR          = int(os.environ.get("SHORT_PYR",          "0"))  # varsayılan KAPALI (bear piyasada SHORT_PYR=1 ile aç)
SHORT_PYR_MAX      = int(os.environ.get("SHORT_PYR_MAX",      "2"))     # max ekleme sayısı
SHORT_PYR_SIZE     = float(os.environ.get("SHORT_PYR_SIZE",   "0.35"))  # 1. ekleme boyutu (orijinalin %35'i — daha temkinli)
SHORT_PYR_BARS     = int(os.environ.get("SHORT_PYR_BARS",     "4"))     # min bar bekle
SHORT_PYR_PROFIT   = float(os.environ.get("SHORT_PYR_PROFIT", "0.008")) # sinyal piramit min kâr
# Kâr-bazlı eşikler: %5 ve %12 — güçlü teyit gerektirir (zayıf düşüşte pyramid yok)
SHORT_PYR_THRESH1  = float(os.environ.get("SHORT_PYR_THRESH1","0.05"))  # 1. ekleme eşiği
SHORT_PYR_THRESH2  = float(os.environ.get("SHORT_PYR_THRESH2","0.12"))  # 2. ekleme eşiği
M1_SHORT_PYR_BEAR_ONLY = bool(int(os.environ.get("M1_SHORT_PYR_BEAR_ONLY", "1")))
M1_SHORT_PYR_MIN_REGIME = float(os.environ.get("M1_SHORT_PYR_MIN_REGIME", "0.20"))

# ── M1 v41 — Adaptif kalite/çıkış katmanı ───────────────────────────────────
# Sadece varsayılan M1 yolunda aktiftir; M4/M5/M7/M8 deney modlarını değiştirmez.
M1_ADAPTIVE_PROFILE = bool(int(os.environ.get("M1_ADAPTIVE_PROFILE", "1")))
M1_BREADTH_CONTROL  = bool(int(os.environ.get("M1_BREADTH_CONTROL", "0")))  # opsiyonel: 3-dönem testte boğayı zayıflattı
M1_TQ_SIZING        = bool(int(os.environ.get("M1_TQ_SIZING", "0")))        # opsiyonel: sizing yerine sinyal skorunda kalite yeterli
M1_CHANDELIER       = bool(int(os.environ.get("M1_CHANDELIER", "1")))
M1_CHAND_N          = int(os.environ.get("M1_CHAND_N", "22"))
M1_BREAKEVEN_ON     = bool(int(os.environ.get("M1_BREAKEVEN_ON", "0")))     # opsiyonel: boğada kazananları erken kesti
M1_BREAKEVEN_R      = float(os.environ.get("M1_BREAKEVEN_R", "1.0"))
M1_REENTRY_COOLDOWN = bool(int(os.environ.get("M1_REENTRY_COOLDOWN", "1"))) # 1: saatlik whipsaw cooldown; coin-pause ayrı kapalı
M1_NORMAL_EXIT_COOLDOWN = bool(int(os.environ.get("M1_NORMAL_EXIT_COOLDOWN", "0"))) # 0: normal çıkış sonrası sinyal varsa tekrar gir
M1_SL_REENTRY_AS_SIZE = bool(int(os.environ.get("M1_SL_REENTRY_AS_SIZE", "1"))) # 1: SL cooldown sinyali bloklamaz, boyutu küçültür
M1_SL_REENTRY_SIZE_MULT = float(os.environ.get("M1_SL_REENTRY_SIZE_MULT", "0.45"))
M1_ROLLING_LOSS_BLOCK = bool(int(os.environ.get("M1_ROLLING_LOSS_BLOCK", "1"))) # 1: 30g aşırı zarar blok; coin-pause ayrı kapalı
M1_ROLLING_LOSS_AS_SIZE = bool(int(os.environ.get("M1_ROLLING_LOSS_AS_SIZE", "0"))) # opsiyonel: işlem kesme yerine deneme boyutu
M1_ROLLING_LOSS_SIZE_MULT = float(os.environ.get("M1_ROLLING_LOSS_SIZE_MULT", "0.35"))
M1_LONG_SL_CLUSTER_AS_SIZE = bool(int(os.environ.get("M1_LONG_SL_CLUSTER_AS_SIZE", "0")))
M1_LONG_SL_CLUSTER_SIZE_MULT = float(os.environ.get("M1_LONG_SL_CLUSTER_SIZE_MULT", "0.40"))
M1_PROFIT_LOCK      = bool(int(os.environ.get("M1_PROFIT_LOCK", "0")))       # opsiyonel: boğada kârı erken kesti
M1_LOCK_MIN_MFE     = float(os.environ.get("M1_LOCK_MIN_MFE", "0.045"))      # önce en az %4.5 lehimize gitsin
M1_LOCK_GIVEBACK    = float(os.environ.get("M1_LOCK_GIVEBACK", "0.55"))     # kârın %55'i geri verilirse çık
M1_LOCK_MIN_GB      = float(os.environ.get("M1_LOCK_MIN_GB", "0.025"))       # min %2.5 mutlak geri verme
M1_LOCK_RETAIN      = float(os.environ.get("M1_LOCK_RETAIN", "0.006"))       # hâlâ en az %0.6 kâr kalsın
M1_LOCK_MIN_BARS    = int(os.environ.get("M1_LOCK_MIN_BARS", "6"))
M1_LOCK_RISK_OFF_ONLY = bool(int(os.environ.get("M1_LOCK_RISK_OFF_ONLY", "1")))
M1_LOCK_RISK_BOOST  = float(os.environ.get("M1_LOCK_RISK_BOOST", "0.20"))    # STRONG_BEAR seviyesi
M1_BEAR_LONG_RISK_MULT = float(os.environ.get("M1_BEAR_LONG_RISK_MULT", "0.45"))
M1_STRONG_BEAR_LONG_RISK_MULT = float(os.environ.get("M1_STRONG_BEAR_LONG_RISK_MULT", "0.25"))
M1_LONG_PYR         = bool(int(os.environ.get("M1_LONG_PYR", "0")))           # kazanan LONG'a ekleme
M1_LONG_PYR_MAX     = int(os.environ.get("M1_LONG_PYR_MAX", "2"))
M1_LONG_PYR_THRESH1 = float(os.environ.get("M1_LONG_PYR_THRESH1", "0.06"))
M1_LONG_PYR_THRESH2 = float(os.environ.get("M1_LONG_PYR_THRESH2", "0.14"))
M1_LONG_PYR_SIZE1   = float(os.environ.get("M1_LONG_PYR_SIZE1", "0.35"))
M1_LONG_PYR_SIZE2   = float(os.environ.get("M1_LONG_PYR_SIZE2", "0.20"))
M1_LONG_PYR_ADX     = float(os.environ.get("M1_LONG_PYR_ADX", "28"))
M1_LONG_PYR_TQ      = float(os.environ.get("M1_LONG_PYR_TQ", "0.58"))
M1_LONG_PYR_CASH_CAP= float(os.environ.get("M1_LONG_PYR_CASH_CAP", "0.10"))
M1_SIG_PYR          = bool(int(os.environ.get("M1_SIG_PYR", "0")))          # aynı coinde tekrar sinyal → kazanana ekle
M1_SIG_PYR_MAX      = int(os.environ.get("M1_SIG_PYR_MAX", "2"))
M1_SIG_PYR_BARS     = int(os.environ.get("M1_SIG_PYR_BARS", "4"))
M1_SIG_PYR_MIN_PROFIT = float(os.environ.get("M1_SIG_PYR_MIN_PROFIT", "0.006"))
M1_SIG_PYR_SIZE     = float(os.environ.get("M1_SIG_PYR_SIZE", "0.45"))
M1_SIG_PYR_SHORT_SIZE = float(os.environ.get("M1_SIG_PYR_SHORT_SIZE", "0.30"))
M1_SIG_PYR_ADX      = float(os.environ.get("M1_SIG_PYR_ADX", "25"))
M1_SIG_PYR_TQ       = float(os.environ.get("M1_SIG_PYR_TQ", "0.56"))
M1_SIG_PYR_MAX_ALLOC= float(os.environ.get("M1_SIG_PYR_MAX_ALLOC", "0.28"))
M1_SIG_PYR_CASH_CAP = float(os.environ.get("M1_SIG_PYR_CASH_CAP", "0.12"))
M1_TREND_HOLD       = bool(int(os.environ.get("M1_TREND_HOLD", "0")))        # güçlü trendde strategy_exit'i bastır
M1_HOLD_MIN_PROFIT  = float(os.environ.get("M1_HOLD_MIN_PROFIT", "0.012"))
M1_HOLD_ADX         = float(os.environ.get("M1_HOLD_ADX", "28"))
M1_HOLD_TQ          = float(os.environ.get("M1_HOLD_TQ", "0.60"))
M1_VOL_QUALITY_SIZING = bool(int(os.environ.get("M1_VOL_QUALITY_SIZING", "1")))
M1_VOLQ_BEAR_ONLY = bool(int(os.environ.get("M1_VOLQ_BEAR_ONLY", "1")))
M1_VOLQ_ATR_RATIO1 = float(os.environ.get("M1_VOLQ_ATR_RATIO1", "0.035"))
M1_VOLQ_ATR_RATIO2 = float(os.environ.get("M1_VOLQ_ATR_RATIO2", "0.055"))
M1_VOLQ_TQ1        = float(os.environ.get("M1_VOLQ_TQ1", "0.62"))
M1_VOLQ_TQ2        = float(os.environ.get("M1_VOLQ_TQ2", "0.70"))
M1_VOLQ_SCALE1     = float(os.environ.get("M1_VOLQ_SCALE1", "0.65"))
M1_VOLQ_SCALE2     = float(os.environ.get("M1_VOLQ_SCALE2", "0.45"))

# LEVER — EMA STACK (Golden Cross) FİLTRESİ
# 0=kapalı (varsayılan, mevcut davranış)
# 1=EMA50>EMA200 zorunlu (bounce-in-downtrend engeli)
# 2=EMA50>EMA200 + MTF(1h) filtresi aktif (çift katman)
M5_EMA_STACK = int(os.environ.get("M5_EMA_STACK", "0"))
...
def _market_breadth(sym_ind: dict[str, pd.DataFrame], ts: pd.Timestamp) -> dict[str, float]:
    """
    Portföy evreninin anlık risk-on/risk-off genişliği.

    Tek bir coine sabit threshold vermek yerine, long/short riskini tüm evrenin
    beraber yukarı mı aşağı mı aktığına göre ayarlamak için kullanılır.
    """
    above, pos_mom, regimes, tq_up, tq_dn = [], [], [], [], []
    for sym, df in sym_ind.items():
        if sym == "BTC/USDT" or ts not in df.index:
            continue
        row = df.loc[ts]
        close = float(row.get("close", 0.0) or 0.0)
        ema = float(row.get("ema_slow", row.get("ema_200", row.get("ema800", 0.0))) or 0.0)
        if close > 0 and ema > 0 and not pd.isna(ema):
            above.append(1.0 if close > ema else 0.0)
        tsmom = row.get("tsmom", np.nan)
        if not pd.isna(tsmom):
            pos_mom.append(1.0 if float(tsmom) > 0 else 0.0)
        regime = row.get("regime_score", np.nan)
        if not pd.isna(regime):
            regimes.append(float(regime))
        u = row.get("trend_quality_up", np.nan)
        d = row.get("trend_quality_down", np.nan)
        if not pd.isna(u):
            tq_up.append(float(u))
        if not pd.isna(d):
            tq_dn.append(float(d))

    above_frac = float(np.mean(above)) if above else 0.5
    pos_mom_frac = float(np.mean(pos_mom)) if pos_mom else 0.5
    median_regime = float(np.median(regimes)) if regimes else 0.5
    risk_on = float(np.clip(0.45 * above_frac + 0.35 * pos_mom_frac + 0.20 * median_regime, 0.0, 1.0))
    return {
        "above_ema_frac": above_frac,
        "positive_mom_frac": pos_mom_frac,
        "median_regime": median_regime,
        "risk_on": risk_on,
        "avg_tq_up": float(np.mean(tq_up)) if tq_up else 0.5,
        "avg_tq_down": float(np.mean(tq_dn)) if tq_dn else 0.5,
    }
...
def fetch_ohlcv(symbol: str, days: int = 365, timeframe: str = "1h",
    ...
    # TF-aware warmup: 15m → EMA800 (960 bar=10 gün), 1h → EMA200 (210 bar)
    _wb = _TF_WARMUP_BARS.get(timeframe, WARMUP_BARS)
    _warmup_min = _wb * _tf_to_minutes(timeframe)
    # Warmup buffer: indikatör ısınması (EMA200/800, HTF, tsmom) için pencere öncesi veri.
    ...
    if start_date is not None:
        ...
        # v13: TF-aware warmup (1h: 210h, 15m: 960×15m=10g, 1m: 300×1m=5h)
        _wb2 = _TF_WARMUP_BARS.get(timeframe, WARMUP_BARS)
        df = df[df.index >= cutoff_ts - pd.Timedelta(minutes=_wb2 * _tf_to_minutes(timeframe))]
...
def prepare_indicators(df: pd.DataFrame, indicators: TechnicalIndicators, timeframe: str = "1h") -> pd.DataFrame:
    ...
    ema_slow_col = indicators.ema_slow_col  # "ema_200" (1h) veya "ema_800" (15m kalibre)
    if ema_slow_col in df_ind.columns and "ema_slow" not in df_ind.columns:
        df_ind["ema_slow"] = df_ind[ema_slow_col]
    # v37 FIX: "ema_fast" alias ekle — bazı kontroller ("ema_50", "ema_fast") sütun adıyla arar.
    # 15m kalibre modda ema_fast=200 → sütun adı "ema_200" (1h'nin "ema_slow" ile çakışır ama değer farklı).
    # Alias sayesinde post-signal EMA50 filtresi (satır 2421+) doğru sütunu bulur.
    ema_fast_col = indicators.ema_fast_col  # "ema_50" (1h) veya "ema_200" (15m kalibre)
    if ema_fast_col in df_ind.columns and "ema_fast" not in df_ind.columns:
        df_ind["ema_fast"] = df_ind[ema_fast_col]
...
def make_strategy(symbol: str, wfo_params: Optional[dict] = None, coin_df: Optional[pd.DataFrame] = None,
    ...
    strat_keys = {
        ...
        "ema_stack_required",
        "medium_momentum_bars", "medium_momentum_pct",
        "slope_bars", "momentum_lookback", "adx_boost",
        "regime_trending_threshold", "regime_ranging_threshold",
        # should_exit TF-aware tampon parametreleri (v37)
        "se_short_buf_mult", "se_long_buf_up", "se_long_buf_dn",
        "se_ema_exit_up", "se_ema_exit_dn",
    }
    ...
    if timeframe == "15m":
        # v37 KALİBRASYON: Tüm indikatör periyotları ×4 (1h eşdeğeri zaman penceresi)
        # Sonuç: ATR/ADX/MACD değerleri 1h ile karşılaştırılabilir → mevcut PROFILES parametreleri çalışır.
        # ─────────────────────────────────────────────────────────────────────────
        # slope_bars: 1h=20bar(20saat) → 15m=80bar(20saat)
        # momentum_lookback: 1h=720bar(30gün) → 15m=2880bar(30gün)
        strat_params.setdefault("slope_bars", 80)           # 80×15m = 20h ≈ 1h slope_bars=20
        strat_params.setdefault("momentum_lookback", 2880)  # 2880×15m = 30gün ≈ 1h momentum_lookback=720
        strat_params["short_momentum_lookback"] = 480       # 5 gün @ 15m (önceden kalibre, korunur)
        strat_params["short_mom_pct"] = 0.95                # 5 günde %5+ düşüş gerekli
        strat_params["short_ema_pct"] = 0.985               # NEUTRAL başlangıcı için güvenli değer
        # should_exit tampon ölçekleme: 1h=1.5/3.5/2.5/4.5/3.0 → 15m=3.0/7.0/5.0/9.0/6.0 (×2)
        # Amaç: 15m ATR56 ≈ 1h ATR14/2 → tampon mesafeleri % olarak eşdeğer (erken SE önler)
        strat_params.setdefault("se_short_buf_mult", 3.0)   # 15m: 1.5×2 = 3.0×ATR56 ≈ 1h 1.62%
        strat_params.setdefault("se_long_buf_up",    7.0)   # 15m: 3.5×2 = 7.0×ATR56
        strat_params.setdefault("se_long_buf_dn",    5.0)   # 15m: 2.5×2 = 5.0×ATR56
        strat_params.setdefault("se_ema_exit_up",    9.0)   # 15m: 4.5×2 = 9.0×ATR56
        strat_params.setdefault("se_ema_exit_dn",    6.0)   # 15m: 3.0×2 = 6.0×ATR56

        # v26: 14 günlük orta-vadeli momentum filtresi — bear bounce tuzağını engeller
        if "medium_momentum_bars" not in strat_params or strat_params["medium_momentum_bars"] == 0:
            strat_params["medium_momentum_bars"] = 1344    # 14 gün @ 15m
            strat_params["medium_momentum_pct"]  = 0.90   # -%10 eşiği

        # EMA Stack (M5_EMA_STACK env flag) — 15m'de bounce-in-downtrend filtresi
        if M5_EMA_STACK >= 1:
            strat_params["ema_stack_required"] = True
        if M5_EMA_STACK >= 2:
            strat_params["mtf_filter_enabled"] = True

    # ── 15m ATR Ölçekleme ────────────────────────────────────────────────────
    # Ölçüm: 15m ATR56 ≈ 1h ATR14 / 2 (EWM smoothing + küçük 15m bar boyutu)
    # Etkisi: aynı çarpan (2.5×) 15m'de 1h'dan 2× dar stop → çok fazla SL hit.
    # Düzeltme: atr_stop_multiplier ve trailing_stop_atr_multiplier × 2.
    # ADX56@15m ≈ ADX14@1h × 0.55 (daha smooth → daha düşük peak değerler).
    # Düzeltme: adx_threshold × 0.55 (PROFILES 1h kalibreli → 15m'de çok yüksek).
    if timeframe == "15m":
        # Ölçüm sonuçları: 15m ATR56 / 1h ATR14 ≈ 0.49 (2.06× fark)
        # Stop için ×2 yeterli (aynı % mesafe), trail için ×3 gerekli (1h trail ≈ 10% → 15m 21×0.524%=11%)
        # R:R simetri: avg_win/avg_loss hedefi ≥1.5 → trail yeterince geniş olmalı
        _ATR_STOP_SCALE  = 2.0    # Initial stop: 2.62% (≈ 1h 2.7%) ✓
        _ATR_TRAIL_SCALE = 3.0    # Trailing: 21×0.524%=11% (≈ 1h 10.26%) → kazananı tutmak için geniş
        _ADX_SCALE = 0.55   # ADX eşik ölçeği (15m ADX56 ≈ 1h ADX14 × 0.55)
        _POS_SCALE = 0.5    # LONG pozisyon ölçeği (dar stop → büyük pozisyon → risk fazla)
        for _k in ("atr_stop_multiplier",):
            if _k in risk_params:
                risk_params[_k] = risk_params[_k] * _ATR_STOP_SCALE
        for _k in ("trailing_stop_atr_multiplier",):
            if _k in risk_params:
                risk_params[_k] = risk_params[_k] * _ATR_TRAIL_SCALE
        if "adx_threshold" in strat_params:
            strat_params["adx_threshold"] = max(12, int(strat_params["adx_threshold"] * _ADX_SCALE))
        # Pozisyon boyutu: 15m dar ATR → büyük pozisyon → konsantrasyon riski yüksek.
        # LONG için max_position_pct × 0.5 — SHORT için aynı kalır (BEAR'da SHORT büyük kâr potansiyeli).
        # Bu ayrım portfolio loop'ta is_short_signal kontrolüyle yapılır (15m_long_pos_scale).
        # Burada sadece min_stop_pct ayarlanır.
        risk_params.setdefault("min_stop_pct", 0.015)  # 15m: min %1.5 stop (dar ATR gürültüsünü aşacak)
        # 15m LONG cap sinyali: portfolio loop bu bayrağı okur
        risk_params["_15m_long_scale"] = _POS_SCALE    # 0.5 = LONG için yarı pozisyon

    # ── TF-aware TechnicalIndicators ──────────────────────────────────────────
    # 15m kalibrasyonu: tüm periyotlar ×4 → 1h ile aynı zaman penceresini kapsar.
    # ATR56@15m ≈ ATR14@1h / 2 (EWM smoothing farkı) — make_strategy'de ×2 scale uygulandı.
    if timeframe == "15m":
        inds = TechnicalIndicators(
            ema_fast=200,           # 200×15m = 50h  ≡ EMA50×1h  (50h)
            ema_slow=800,           # 800×15m = 200h ≡ EMA200×1h (200h)
            atr_period=56,          # 56×15m  = 14h  ≡ ATR14×1h  → stop mesafesi 1h ile eşdeğer
            adx_period=56,          # 56×15m  = 14h  ≡ ADX14×1h  → trend gücü eşdeğer
            rsi_period=56,          # 56×15m  = 14h  ≡ RSI14×1h  → should_exit erken ateşlemez
            volume_sma_period=80,   # 80×15m  = 20h  ≡ VolSMA20×1h
            macd_fast=48,           # 48×15m  = 12h  ≡ MACD12×1h → gürültü elenir
            macd_slow=104,          # 104×15m = 26h  ≡ MACD26×1h
            macd_signal=36,         # 36×15m  = 9h   ≡ MACD9×1h
            bb_period=80,           # 80×15m  = 20h  ≡ BB20×1h
            donchian_period=80,     # 80×15m  = 20h  ≡ Donchian20×1h
            tsmom_period=80,        # 80×15m  = 20h  ≡ tsmom20×1h
        )
    else:
        inds = TechnicalIndicators()
...
class PPos:
    ...
    # M1 v41 diagnostics — trade içi en iyi/en kötü fiyatlar
...
def select_active_coins(
    ...
    for sym, df in sym_ind.items():
        if sym in ("BTC/USDT", "_BTC_REGIME_"):
            continue
        if len(df) < 100:   # Minimum 100 bar (ADX/ATR/Hurst için yeterli)
            continue
        try:
...
def _assess_btc_regime(btc_df: pd.DataFrame, trade_start: pd.Timestamp,
    ...
    # v37: TF-aware span ölçekleme — 15m barlarda 200-span = 50h (çok kısa), 1h eşdeğeri gerekli.
    # 1h:  ema200=200h, ema720=720h(30d), ema168=168h(7d) → referans zaman pencereleri
    # 15m: ×4 → ema200→800, ema720→2880, ema168→672 bar
    _scale = _bars_per_day(timeframe) // 24  # 1h→1, 15m→4, 1m→60
    _span200 = max(200, 200 * _scale)
    _span720 = max(720, 720 * _scale)
    _span168 = max(168, 168 * _scale)
    _min_bars = _span200
    if len(pre) < _min_bars:
        return "NEUTRAL"
    ...
    # BTC'nin son konumu: scaled EMA spans ile (1h eşdeğeri zaman pencereleri)
    ema200  = close.ewm(span=_span200, adjust=False).mean()
    ema720  = close.ewm(span=_span720, adjust=False).mean()
    ema168  = close.ewm(span=_span168, adjust=False).mean()
...
def _run_regime_checkpoint(
    ...
    if use_universe:
        # Tüm rejimlerde universe'den skor bazlı seçim yap
        # BEAR'da coin sayısı azalır (REGIME_MAX_COINS) ama tamamen sabit listeye dönülmez
...
def run_portfolio_backtest(
    ...
    use_universe: bool = True,      # True → UNIVERSE'den dinamik seçim (varsayılan: 26 coin)
    ...
    if timeframe is None:
        ...
        if _tf_env:
            ...
            timeframe = "1h"
    _m1_default_mode = not (m4_mode or m5_mode or m6_mode or _is_m7 or _is_m8)
    ...
    # SHORT-only coinleri her zaman ekle (BEAR rejimde SHORT için gerekli)
    for _so in SHORT_ONLY_UNIVERSE:
        if _so not in fetch_list:
            fetch_list = list(fetch_list) + [_so]
    ...
    # v37: TF-aware TechnicalIndicators — sym_ind ile make_strategy aynı indikatör sütunlarını kullanmalı.
    # 15m: ema_fast=200 (50h), ema_slow=800 (200h) → strateji bu sütunları arar.
    # DEFAULT indicators_obj yalnızca ema_50/ema_200 üretiyor → strategy HOLD döndürürdü (sütun eksik).
    if timeframe == "15m":
        indicators_obj = TechnicalIndicators(
            ema_fast=200,  ema_slow=800, atr_period=56,  adx_period=56,
            volume_sma_period=80, macd_fast=48, macd_slow=104, macd_signal=36,
            bb_period=80,  donchian_period=80, tsmom_period=80,
        )
    else:
        indicators_obj = TechnicalIndicators()
    sym_ind: dict[str, pd.DataFrame] = {}
    ...
    if auto_mode:
        ...
        if _btc_for_regime is not None:
            detected = _assess_btc_regime(_btc_for_regime, trade_start, timeframe=timeframe)
        else:
            detected = "NEUTRAL"
    ...
    # ── Adaptif Coin Profilleri ─────────────────────────────────────────────
    # Her coin için son işlem geçmişini + anlık piyasa durumunu birleştirerek
    # dinamik giriş eşiği hesaplayan öğrenme sistemi.
    # Sabit threshold yerine bot kendi kötü/iyi dönemlerinden adapte olur.
    adaptive_profiles: dict[str, AdaptiveCoinProfile] = {}
    for _sym in strategies:
        _s = strategies[_sym]
        adaptive_profiles[_sym] = AdaptiveCoinProfile(
            sym               = _sym,
            base_entry_trend  = _s.ENTRY_SCORE_TREND,
            base_entry_ranging= _s.ENTRY_SCORE_RANGING,
            window            = 8,    # Son 8 işlem: yakın dönem ağırlıklı
            max_adj           = 0.12, # Maksimum ±0.12 değişim
        )
    ...
    # v38: BULL→BEAR geçiş koruyucusu
    # Son 36 saatte 2+ LONG SL olursa yeni LONG girişleri 12 saat durdur.
    # Neden: Mayıs 2026'da ATOM(12)/NEAR(13)/XRP(14) art arda SL yedi.
    # Piyasa dönerken bot hâlâ LONG açıyordu (rejim gecikmeli güncellendi).
    _long_sl_times: list = []   # Son LONG SL zaman damgaları
    ...
    # (coin_sl_history removed - was causing side effects)
    ...
    if btc_ind is not None and len(btc_ind) > 0:
        ...
        _init_btc_regime = _assess_btc_regime(_btc_pre_start, trade_start, timeframe=timeframe)
    ...
    for ts in all_ts:
        ...
        if day_str != _last_day:
            ...
            if _btc_m1_dynamic and btc_ind is not None and (
                ...
                _new_btc_regime = _assess_btc_regime(_btc_slice_now, ts, timeframe=timeframe)
        ...
        if btc_ind is not None and ts in btc_ind.index:
            ...
            for sym, strat in strategies.items():
                ...
                # v35: Rejime göre SHORT tetik eşiğini dinamik güncelle
                # BEAR/STRONG_BEAR: 0.998 (gevşek — yavaş düşen büyük cap'larda SHORT)
                # NEUTRAL/BULL: 0.985 (katı — bull recovery'de yanlış SHORT önlenir)
                _is_bear_regime = _current_regime_params.entry_score_boost >= 0.11
                for _strat in strategies.values():
                    _strat.apply_short_regime(_is_bear_regime)
        ...
        for sym, pos in list(open_positions.items()):
            ...
            high_px = float(row.get("high", price) or price)
            low_px  = float(row.get("low", price) or price)
            if pos.max_price_seen <= 0:
                pos.max_price_seen = pos.entry_price
            if pos.min_price_seen <= 0:
                pos.min_price_seen = pos.entry_price
            pos.max_price_seen = max(pos.max_price_seen, high_px)
            pos.min_price_seen = min(pos.min_price_seen, low_px)

            if pos.is_short:
                # ── SHORT: trailing stop AŞAĞI iner (fiyat düştükçe)
                if _m1_default_mode and M1_CHANDELIER and "low" in df.columns:
                    _ll = float(df["low"].loc[:ts].iloc[-M1_CHAND_N:].min())
                    new_trail = _ll + pos.trailing_mult * atr
                else:
                ...
                if (_m1_default_mode and M1_BREAKEVEN_ON and pos.r_value > 0
                        and (pos.entry_price - price) >= pos.r_value * M1_BREAKEVEN_R):
                ...
                # ── SHORT Pyramid (Kâr-Bazlı) ────────────────────────────────────
                # Fiyat düştükçe ek lot ekle (Turtle "add-to-winners" — kısa yön).
                # Guard: BULL rejimde SHORT pyramid yok (boğa piyasasında short averajlamak tehlikeli).
                # ADX ≥ 30: güçlü düşüş trendi onayı gerekli.
                _short_not_bull = not (_current_regime_params.entry_score_boost <= -0.03)  # BULL/STRONG_BULL değil
                _m1_short_pyr_regime_ok = (
                    (not _m1_default_mode)
                    or (not M1_SHORT_PYR_BEAR_ONLY)
                    or (_current_regime_params.entry_score_boost >= M1_SHORT_PYR_MIN_REGIME)
                )
                if (SHORT_PYR and atr > 0 and pos.pyramid_count < SHORT_PYR_MAX
                        and _short_not_bull and _m1_short_pyr_regime_ok):
                    _s_adx = float(row.get("adx", 0.0))
                    _spnl  = (pos.entry_price - price) / pos.entry_price if pos.entry_price > 0 else 0.0
                    _s_thresholds = [SHORT_PYR_THRESH1, SHORT_PYR_THRESH2]
                    _s_sizes      = [SHORT_PYR_SIZE, SHORT_PYR_SIZE * 0.50]
                    for _spi, _spthresh in enumerate(_s_thresholds):
                        if _spnl >= _spthresh and pos.pyramid_count == _spi:
                            if _s_adx < 30:  # Sadece güçlü trendde ekleme yap (ADX≥30)
                                break
                            _s_add_size = pos.size_at_entry * _s_sizes[_spi]
                            _s_add_fill = price * (1 - SLIPPAGE)
                            _s_add_comm = _s_add_fill * _s_add_size * COMMISSION
                            _s_add_cost = _s_add_fill * _s_add_size + _s_add_comm
                            if _s_add_cost >= MIN_ORDER_SIZE and _s_add_cost <= balance * 0.10:
                                # Ağırlıklı ortalama giriş fiyatını güncelle
                                # → PnL hesabı tüm lot için doğru olur
                                _s_old_size = pos.size
                                _s_new_size = _s_old_size + _s_add_size
                                pos.entry_price = (
                                    (pos.entry_price * _s_old_size + _s_add_fill * _s_add_size)
                                    / _s_new_size
                                )
                                pos.size   = _s_new_size
                                pos.cost  += _s_add_cost   # toplam marjin artar
                                balance   -= _s_add_cost
                                pos.pyramid_count = _spi + 1
                                # SHORT stop: kâr koru — stop'u yeni fiyatın 2×ATR üstüne çek
                                _s_new_stop = price + 2.0 * atr
                                if _s_new_stop < pos.stop_price:
                                    pos.stop_price = _s_new_stop
                            break
            else:
                ...
                elif _m1_default_mode and M1_CHANDELIER and "high" in df.columns:
                    _hh = float(df["high"].loc[:ts].iloc[-M1_CHAND_N:].max())
                    new_trail = _hh - _zoom_trail * atr
                else:
                    new_trail = price - _zoom_trail * atr
                if new_trail > pos.trail_price:
                    pos.trail_price = new_trail
                if (_m1_default_mode and M1_BREAKEVEN_ON and pos.r_value > 0
                        and (price - pos.entry_price) >= pos.r_value * M1_BREAKEVEN_R):
                ...
                _tq_pyr_up = float(row.get("trend_quality_up", 0.5) or 0.5)
                _pyr_m1 = (_pyr_base and _m1_default_mode and M1_LONG_PYR
                           and pos.pyramid_count < M1_LONG_PYR_MAX
                           and _btc_m1_active
                           and _current_regime_params.entry_score_boost <= -0.03
                           and _adx_for_pyramid >= M1_LONG_PYR_ADX
                           and _tq_pyr_up >= M1_LONG_PYR_TQ)
                if _pyr_m6 or _pyr_m7 or _pyr_m4 or _pyr_m1:
                    if m6_mode:
                        ...
                        _pyramid_thresholds = [M1_LONG_PYR_THRESH1, M1_LONG_PYR_THRESH2]
                        _pyramid_sizes      = [M1_LONG_PYR_SIZE1, M1_LONG_PYR_SIZE2]
                        _pyr_cash_cap       = M1_LONG_PYR_CASH_CAP
                    elif _is_m7:
                ...
                if len(_cl) >= M7_LOCK_DOWN_BARS + 1:
                    ...
                    if all(_seq[i] < _seq[i - 1] for i in range(1, len(_seq))):  # N mum art arda düşüş
                        ...
                        if not (M7_LOCK_SKIP_STRONG and _coin_still_pumping(df.loc[:ts], adx_min=M7_HOLD_ADX)):
                            hit_profitlock = True
            if (_m1_default_mode and M1_PROFIT_LOCK and pos.entry_price > 0
                    and (not M1_LOCK_RISK_OFF_ONLY or _current_regime_params.entry_score_boost >= M1_LOCK_RISK_BOOST)
                    and pos.bars_held >= M1_LOCK_MIN_BARS):
                if pos.is_short:
                    _m1_mfe = (pos.entry_price - pos.min_price_seen) / pos.entry_price
                    _m1_cur = (pos.entry_price - price) / pos.entry_price
                    _m1_tq = float(row.get("trend_quality_down", 0.5) or 0.5)
                else:
                    _m1_mfe = (pos.max_price_seen - pos.entry_price) / pos.entry_price
                    _m1_cur = (price - pos.entry_price) / pos.entry_price
                    _m1_tq = float(row.get("trend_quality_up", 0.5) or 0.5)
                _m1_adx = float(row.get("adx", 0.0) or 0.0)
                _m1_gb = _m1_mfe - _m1_cur
                _m1_gb_ratio = M1_LOCK_GIVEBACK
                if _m1_adx >= 35 and _m1_tq >= 0.65:
                    _m1_gb_ratio = min(0.85, _m1_gb_ratio + 0.20)
                if (_m1_mfe >= M1_LOCK_MIN_MFE
                        and _m1_cur >= M1_LOCK_RETAIN
                        and _m1_gb >= max(M1_LOCK_MIN_GB, _m1_mfe * _m1_gb_ratio)):
                    hit_profitlock = True
            ...
            if hit_stop or hit_trail or hit_tp or hit_time or hit_maxhold or hit_fastexit or hit_profitlock or hit_stagnation:
                ...
                if pos.is_short:
                    ...
                    # NOT: pyramid yapıldıysa pos.entry_price ağırlıklı ortalamadır,
                    #      pos.cost tüm marjini içerir (orijinal + pyramid eklemeleri).
                    exit_px      = price * (1 + SLIPPAGE)
                    exit_comm    = exit_px * pos.size * COMMISSION
                    gross_pnl    = (pos.entry_price - exit_px) * pos.size  # avg entry kullanılır
                    net_pnl      = gross_pnl - exit_comm
                    pos.exit_price  = exit_px
                    pos.exit_time   = ts
                    pos.exit_reason = reason
                    pos.pnl         = net_pnl                # pyramid zaten cost'a dahil
                    balance        += pos.cost + net_pnl     # tüm marjini geri al + kâr/zarar
                else:
                ...
                if reason == "stop_loss" and not pos.is_short:
                    ...
                    # v38: Portfolio geçiş koruyucusu — LONG SL'leri takip et
                    _long_sl_times.append(ts)
                    _long_sl_times = [t for t in _long_sl_times
                                      if ts - t < pd.Timedelta(hours=36)]
                else:
                ...
                # Adaptif profil güncelle (SL/TS çıkışı)
                if sym in adaptive_profiles and pos.cost > 0:
                    _dur_h = (pos.exit_time - pos.entry_time).total_seconds() / 3600 if pos.exit_time and pos.entry_time else 0.0
                    adaptive_profiles[sym].record_trade(
                        pnl         = pos.pnl,
                        pnl_pct     = pos.pnl / pos.cost,
        ...
        for sym in list(open_positions.keys()):
            ...
            slice_df = df.loc[:ts]
            try:
                # v38: entry_price gerçek değerle ilet → minimum kâr kapısı çalışsın
                should_exit, _ = strategies[sym].should_exit(
                    sym, slice_df, entry_price=open_positions[sym].entry_price,
            ...
            if should_exit:
                ...
                if _m1_default_mode and M1_TREND_HOLD:
                    _hpos = open_positions[sym]
                    _hrow = slice_df.iloc[-1]
                    _hprice = float(_hrow.get("close", 0.0) or 0.0)
                    _hadx = float(_hrow.get("adx", 0.0) or 0.0)
                    _hema50_col = next((c for c in ("ema_50", "ema50", "ema_fast") if c in slice_df.columns), None)
                    _hema200_col = next((c for c in ("ema_200", "ema_slow", "ema200") if c in slice_df.columns), None)
                    _hema50 = float(_hrow.get(_hema50_col, _hprice) or _hprice) if _hema50_col else _hprice
                    _hema200 = float(_hrow.get(_hema200_col, _hprice) or _hprice) if _hema200_col else _hprice
                    if _hpos.entry_price > 0 and _hprice > 0:
                        if _hpos.is_short:
                            _hpnl_pct = (_hpos.entry_price - _hprice) / _hpos.entry_price
                            _htq = float(_hrow.get("trend_quality_down", 0.5) or 0.5)
                            _not_bull = not (_current_regime_params.entry_score_boost <= -0.03)
                            if (_hpnl_pct >= M1_HOLD_MIN_PROFIT
                                    and _not_bull
                                    and _hprice < _hema50
                                    and _hprice < _hema200
                                    and _hadx >= M1_HOLD_ADX
                                    and _htq >= M1_HOLD_TQ):
                                continue
                        else:
                            _hpnl_pct = (_hprice - _hpos.entry_price) / _hpos.entry_price
                            _htq = float(_hrow.get("trend_quality_up", 0.5) or 0.5)
                            if (_hpnl_pct >= M1_HOLD_MIN_PROFIT
                                    and (_hpos.is_coin_bull or _hprice > _hema200)
                                    and _hprice > _hema50
                                    and _hadx >= M1_HOLD_ADX
                                    and _htq >= M1_HOLD_TQ):
                                continue
                ...
                if pos.is_short:
                    exit_px   = price * (1 + SLIPPAGE)
                    exit_comm = exit_px * pos.size * COMMISSION
                    gross_pnl = (pos.entry_price - exit_px) * pos.size  # avg entry (pyramid dahil)
                    net_pnl   = gross_pnl - exit_comm
                    pos.exit_price  = exit_px
                    pos.exit_time   = ts
                    pos.exit_reason = "strategy_exit"
                    pos.pnl   = net_pnl              # cost tüm marjini içeriyor
                    balance  += pos.cost + net_pnl   # tüm marjini geri al
                else:
                ...
                strategies[sym].record_outcome(sym, pos.pnl > 0, pnl=pos.pnl)
                regime_ctrl.record_trade(pos.pnl > 0)
                # Adaptif profil güncelle (SE çıkışı)
                if sym in adaptive_profiles and pos.cost > 0:
                    _dur_h = (pos.exit_time - pos.entry_time).total_seconds() / 3600 if pos.exit_time and pos.entry_time else 0.0
                    adaptive_profiles[sym].record_trade(
                        pnl         = pos.pnl,
        ...
        _breadth_now = _market_breadth(sym_ind, ts) if (_m1_default_mode and M1_BREADTH_CONTROL) else {
            "risk_on": 0.5,
            "avg_tq_up": 0.5,
            "avg_tq_down": 0.5,
        }
        ...
        if daily_loss_ok and (len(open_positions) < eff_max or (_m1_default_mode and M1_SIG_PYR)):
            ...
            for sym in candidate_syms:
                ...
                _m5_sig_pyr    = False   # bu iterasyonda M5 signal pyramid modu mu?
                _short_sig_pyr = False   # bu iterasyonda SHORT signal pyramid modu mu?
                _m1_sig_pyr    = False   # M1: açık pozisyonda aynı yön sinyaliyle ek lot
                _m1_reentry_risk_scale = 1.0
                if sym in open_positions:
                    _pos_existing = open_positions[sym]
                    # ── M5 Signal Pyramid ────────────────────────────────────────
                    # Açık LONG pozisyona yeni BUY sinyali gelince ek lot ekle.
                    if (m5_mode and M5_SIG_PYR > 0
                            and not _pos_existing.is_short
                            and getattr(_pos_existing, 'pyramid_count', 0) < M5_SIG_PYR_MAX
                            and _pos_existing.bars_held >= M5_SIG_PYR_BARS):
                        _price_now = float(sym_ind[sym].loc[ts, "close"]) if (sym in sym_ind and ts in sym_ind[sym].index) else 0.0
                        _entry_px  = _pos_existing.entry_price
                        _pnl_ratio = (_price_now - _entry_px) / _entry_px if _entry_px > 0 else 0.0
                        _profit_ok = (M5_SIG_PYR >= 2) or (_pnl_ratio >= M5_SIG_PYR_PROFIT)
                        # V3 rejim kapısı: Regime.BULL veya STRONG_BULL'da ekle
                        # entry_score_boost <= 0 → rejim BULL veya STRONG_BULL
                        _is_bull_regime = _current_regime_params.entry_score_boost <= 0
                        _bull_ok = (not M5_SIG_PYR_BULL_ONLY) or _is_bull_regime
                        if _profit_ok and _bull_ok:
                            _m5_sig_pyr = True
                        else:
                            continue
                    elif (_m1_default_mode and M1_SIG_PYR
                            and getattr(_pos_existing, 'pyramid_count', 0) < M1_SIG_PYR_MAX
                            and _pos_existing.bars_held >= M1_SIG_PYR_BARS
                            and _pos_existing.entry_price > 0):
                        _m1_sig_pyr = True
                    # ── SHORT Signal Pyramid (devre dışı — kâr-bazlı yeterli) ─────
                    # Sinyal bazlı (%0.8 kâr kapısı) çok küçük — WR ve PF'i bozuyor.
                    # Sadece kâr-bazlı (%4/%10) SHORT pyramid aktif.
                    # Yeniden aktifleştirmek için aşağıdaki bloğu yorum dışına al.
                    # M8 LEVER 4 — KADEMELİ GİRİŞ: koşullar sağlanırsa aynı coine ek lot
                    elif (_is_m8 and M8_SCALE_IN
                            and not _pos_existing.is_short           # sadece LONG'a ekle
                            and _pos_existing.pyramid_count < M8_SCALE_MAX_ADDS  # max birim
                            and _pos_existing.bars_held >= M8_SCALE_MIN_BARS     # yeterli bar
                            and _pos_existing.entry_price > 0):      # geçerli giriş fiyatı
                        _m8_scale_in = True  # devam et, filtrelerden geçsin, sonunda ekle
                    else:
                        continue
                if len(open_positions) >= eff_max and not _m8_scale_in and not _short_sig_pyr and not _m1_sig_pyr:
                    if _m1_default_mode and M1_SIG_PYR:
                        continue
                    break
                ...
                # Coinin kendi trend gücü (ADX) — MTF: 15m modu için 1h ADX kullan
                coin_adx_strong = False
                if timeframe == "15m" and "htf_adx" in slice_df.columns:
                    coin_adx_strong = float(slice_df["htf_adx"].iloc[-1]) > 22
                elif "adx" in slice_df.columns:
                ...
                if in_global_bear and coin_own_bull:
                    ...
                    # Sebep: NEAR/INJ gibi altcoin'lerin kendi bull trend'leri BTC bear ile
                    # örtüşebilir. Bu durumda tam +0.11 yerine +0.055 kullanmak daha adil.
                ...
                if rolling_pnl_30d < -COIN_MAX_LOSS:
                    if _m1_default_mode and M1_ROLLING_LOSS_AS_SIZE:
                        _m1_reentry_risk_scale = min(_m1_reentry_risk_scale, M1_ROLLING_LOSS_SIZE_MULT)
                    elif not (_m1_default_mode and not M1_ROLLING_LOSS_BLOCK):
                        continue
                ...
                # ── AdaptiveCoinProfile: passif öğrenme modu ─────────────────────
                # Profil her trade sonrası güncellendi (record_trade çağrıları).
                # Şu an sadece izliyor; status() ile debug/rapor amaçlı kullanılabilir.
                # Entegrasyon aktifleştirildiğinde: adaptive_threshold() veya status()
                # ile pozisyon boyutu/cooldown ayarlanacak. (Mevcut filtreler zaten iyi
                # kalibre olduğu için ek katman şimdilik regresyon oluşturuyor.)
                # _adp_debug = adaptive_profiles.get(sym)
                # if _adp_debug: logger.debug(f"[Adaptif] {sym}: {_adp_debug.status()}")

                # 2) Çıkış sonrası bekleme. M1 default'ta kapalı: kullanıcı isteği
                # "sinyal veriyorsa aynı coine tekrar gir" davranışını korur.
                if not (_m1_default_mode and not M1_REENTRY_COOLDOWN):
                    _sl_hours = coin_risk[sym].get("sl_cooldown_hours", 24)
                    # v39: Güçlü trendde ADX-aware cooldown — SL sonrası 96h → 6h
                    # Neden: INJ/NEAR SL sonrası 96h beklenince trend devam ederken re-entry kaçırıldı.
                    # Guard: Yeni giriş yine ADX, choppiness, momentum filtrelerinden geçmeli (kalite korunur).
                    # ADX anlık: slice_df zaten hazır (satır 2264), sadece son değeri al.
                    _adx_now_reentry = float(slice_df["adx"].iloc[-1]) if "adx" in slice_df.columns else 0.0
                    _is_strong_trend_reentry = _adx_now_reentry >= 35 and coin_own_bull
                    if _is_strong_trend_reentry:
                        _sl_hours = min(_sl_hours, 6)   # Güçlü uptrend: maks 6h SL bekle
                    elif _adx_now_reentry >= 28 and coin_own_bull:
                        _sl_hours = min(_sl_hours, 18)  # Orta-güçlü uptrend: maks 18h bekle
                    if coin_own_bull:
                        ...
                        if sym_last_sl_b is not None and ts - sym_last_sl_b < pd.Timedelta(hours=_sl_hours):
                            if _m1_default_mode and M1_SL_REENTRY_AS_SIZE:
                                _m1_reentry_risk_scale = min(_m1_reentry_risk_scale, M1_SL_REENTRY_SIZE_MULT)
                            else:
                                continue
                        # M4v11: Normal çıkış sonrası 24h→8h (boğa trendinde daha hızlı re-entry)
                        # v39: Güçlü trendde (ADX≥30) 8h → 4h — trailing/SE çıkışı sonrası trend yakalanır
                        _exit_wait_h = 4 if _adx_now_reentry >= 30 and coin_own_bull else 8
                        if M1_NORMAL_EXIT_COOLDOWN or not _m1_default_mode:
                            last_exit_ts = coin_last_exit.get(sym)
                            if last_exit_ts is not None and ts - last_exit_ts < pd.Timedelta(hours=_exit_wait_h):
                                continue
                    else:
                ...
                # M1 v41: Adaptif profil artık pasif değil. Base entry eşiği
                # son trade performansı + anlık mikroyapıdan türetilir; rejim
                # boost'u bunun üstüne eklenir.
                if _m1_default_mode and M1_ADAPTIVE_PROFILE and sym in adaptive_profiles:
                    _adp = adaptive_profiles[sym]
                    _base_tr = _adp.adaptive_threshold(slice_df, is_trending=True)
                    _base_rg = _adp.adaptive_threshold(slice_df, is_trending=False)
                    strategies[sym].apply_regime_params(
                        effective_entry_boost,
                        base_entry_trend=_base_tr,
                        base_entry_ranging=_base_rg,
                    )
                ...
                if signal.side not in (Side.BUY, Side.SHORT):
                    continue
                _tq_up_now = float(row.get("trend_quality_up", 0.5) or 0.5)
                _tq_dn_now = float(row.get("trend_quality_down", 0.5) or 0.5)
                if pd.isna(_tq_up_now):
                    _tq_up_now = 0.5
                if pd.isna(_tq_dn_now):
                    _tq_dn_now = 0.5
                _risk_on_now = float(_breadth_now.get("risk_on", 0.5))

                if _m1_default_mode and M1_BREADTH_CONTROL:
                    if (not is_short_signal) and _risk_on_now < 0.28 and _tq_up_now < 0.68:
                        continue  # evren risk-off + coin trend kalitesi zayıf → LONG alma
                    if is_short_signal and _risk_on_now > 0.72 and _tq_dn_now < 0.68:
                        continue  # evren risk-on + düşüş kalitesi zayıf → SHORT alma

                # ── Yön bazlı post-signal filtreler ──────────────────────
                ema_col = next((c for c in ("ema_50", "ema50", "ema_fast") if c in slice_df.columns), None)

                # v38: BULL→BEAR Geçiş Koruyucusu — sinyal tipini bilince kontrol et
                # Son 36 saatte 2+ LONG SL → yeni LONG girişi 12 saat durdur.
                # Piyasa dönerken rejim gecikmeli güncellenir (Mayıs 12-14: 3 art arda SL).
                if not is_short_signal and _long_sl_times:
                    _recent_long_sls = sum(1 for t in _long_sl_times
                                           if ts - t < pd.Timedelta(hours=36))
                    if _recent_long_sls >= 2:
                        _last_sl_ts = max(_long_sl_times)
                        if ts - _last_sl_ts < pd.Timedelta(hours=12):
                            if _m1_default_mode and M1_LONG_SL_CLUSTER_AS_SIZE:
                                _m1_reentry_risk_scale = min(
                                    _m1_reentry_risk_scale,
                                    M1_LONG_SL_CLUSTER_SIZE_MULT,
                                )
                            else:
                                continue  # Piyasa dönüyor olabilir → yeni LONG 12 saat dur
                ...
                # SHORT-only coinlerde LONG yasak (bu coinler sadece SHORT için)
                if not is_short_signal and sym in SHORT_ONLY_UNIVERSE:
                    continue  # SHORT-only coin → LONG sinyali blokla
                ...
                # v26: BEAR-only → NOT BULL kısıtlamasına gevşetildi (NEUTRAL SHORTs izinli)
                if is_short_signal and in_global_bull:
                    continue  # BULL/STRONG_BULL'da short yasak; NEUTRAL+BEAR izinli

                # SHORT kalite filtresi: devre dışı (declining bar filter 6M/ayı haftasını bozuyor)
                # Sürdürülen bear dönemlerinde normal bounce barlarında da SHORT girilmeli.
                # TODO: sadece aşırı bounce'larda (>1.5%) ve SHORT-only coinde test et
                ...
                if _m1_default_mode:
                    if M1_TQ_SIZING:
                        _tq_for_size = _tq_dn_now if is_short_signal else _tq_up_now
                        combined_mult *= float(np.clip(0.45 + 1.25 * _tq_for_size, 0.40, 1.60))
                    if M1_BREADTH_CONTROL:
                        if is_short_signal:
                            # Risk-on genişliği yükseldikçe short riski küçülür; risk-off'ta short'a alan açılır.
                            combined_mult *= float(np.clip(1.25 - 0.90 * _risk_on_now, 0.30, 1.25))
                        else:
                            # Long tarafı piyasa genişliğiyle taşınır; zayıf genişlikte boyut küçülür.
                            combined_mult *= float(np.clip(0.35 + 1.10 * _risk_on_now, 0.35, 1.35))
                    if (M1_VOL_QUALITY_SIZING and not is_short_signal and price > 0
                            and (not M1_VOLQ_BEAR_ONLY or not in_global_bull)):
                        _atr_ratio_now = atr / price
                        if _atr_ratio_now >= M1_VOLQ_ATR_RATIO2 and _tq_up_now < M1_VOLQ_TQ2:
                            combined_mult *= M1_VOLQ_SCALE2
                        elif _atr_ratio_now >= M1_VOLQ_ATR_RATIO1 and _tq_up_now < M1_VOLQ_TQ1:
                            combined_mult *= M1_VOLQ_SCALE1
                    combined_mult = float(np.clip(combined_mult, 0.08, _mult_cap))
                ...
                if _m1_default_mode and _m1_reentry_risk_scale < 1.0:
                    combined_mult = float(np.clip(combined_mult * _m1_reentry_risk_scale, 0.05, _mult_cap))
                ...
                # v37: 15m LONG pozisyon kısıtı — 15m ATR dar → büyük pozisyon → LONG SL kayıpları büyük.
                # SHORT'a UYGULANMAZ (BEAR SHORTs tam boyutlu kalmalı = büyük kâr potansiyeli).
                if not is_short_signal:
                    _15m_ls = coin_risk[sym].get("_15m_long_scale", 1.0)
                    _max_pos_pct *= _15m_ls

                # ── Rejim-Duyarlı Pozisyon Kapısı (per-coin) ─────────────────────
                # Bazı coinler BEAR'da volatil → büyük LONG pozisyon = büyük kayıp.
                # `bear_max_position_pct` varsa: BEAR'da bu alt sınır kullanılır.
                # BULL'da normal max_position_pct geçerli → star performer kârı korunur.
                # Örnek: TON bear'da %3 cap, bull'da %8 → 6M kârı korurken 1W zararı sınırlar.
                if in_global_bear and not is_short_signal:
                    _bear_cap = coin_risk[sym].get("bear_max_position_pct", None)
                    if _bear_cap is not None:
                        _max_pos_pct = min(_max_pos_pct, _bear_cap)

                # v9: BEAR rejimde TÜM modellerde pos boyutu yarıya
                # v38 SHORT FIX: Onaylı BEAR SHORT (coin_own_bull=False) → tam boyut.
                # Neden: ETH -13% hareket yaptı ama SHORT sadece +$3 kazandı (pozisyon çok küçük).
                # coin_own_bull=False + in_global_bear = "coin de bear" → SHORT tam boyut al.
                if in_global_bear:
                    if is_short_signal and not coin_own_bull:
                        # Onaylı BEAR: coin kendi EMA'sının altında + global bear → tam boyut SHORT
                        pass  # Ceza yok — bu ideal SHORT koşulu
                    elif is_short_signal:
                        _max_pos_pct *= 0.75   # Kısmi bear SHORT (coin hâlâ uptrend'de)
                        risk_pct_adj *= 0.75
                    else:
                        _m1_bear_long_mult = (
                            M1_STRONG_BEAR_LONG_RISK_MULT if in_strong_bear
                            else M1_BEAR_LONG_RISK_MULT
                        )
                        _max_pos_pct *= _m1_bear_long_mult    # LONG: işlem kesme, risk küçült
                        risk_pct_adj *= _m1_bear_long_mult
                if _is_high_conf_bull:
                    if m6_mode:
                ...
                elif is_short_signal and not coin_own_bull and in_global_bear:
                    # v38: Onaylı BEAR SHORT — LONG'un bull boost'una benzer şekilde geniş trail.
                    # ETH -13% düşerken bot +$3 kazandı (trail çok dar, bounce'ta çıktı).
                    # Coin AŞAĞI trendde + global bear → trail genişlet (bounce'a toleranslı ol).
                    _adx_short_trail = 1.0 if _adx_now >= 35 else (0.5 if _adx_now >= 30 else 0.0)
                    trail_mult = max(2.0, base_trail * 1.3 + _adx_short_trail)  # 30% boost
                else:
                ...
                # FIX #1: Minimum tutma süresi — rejime göre dinamik
                # BEAR: min hold SIFIR → erken SE çıkışları koruyucu (kısa bear ralliler)
                # NEUTRAL/BULL: min hold artırıldı → büyük trend hareketleri daha uzun tutulur
                # Analiz: 0-12 saatlik işlemler %0-28 WR; 1-3 gün %59 WR; 3+ gün %93 WR
                _hold_scale = _bpd / 24  # 1h:1, 15m:4, 1m:60
                if in_strong_bear:
                    hold_bars = 0   # STRONG_BEAR: min hold yok, hızlı çıkış (piyasa çöküyor)
                elif in_global_bear:
                    hold_bars = int(8 * _hold_scale)   # BEAR: 8 saat min hold (orta yol)
                else:
                    hold_bars = int((24 if coin_own_bull else 16) * _hold_scale)  # NEUTRAL/BULL
                # M7: scalper → çok kısa min hold (hızlı TP/time-stop çıkışlarına izin ver).
                if m7_mode:
                    hold_bars = M7_MIN_HOLD_BARS

                # ── M1 Signal Pyramid — aynı yönde tekrar sinyal gelirse kazanana ekle ──
                if _m1_sig_pyr:
                    _pos_m1 = open_positions[sym]
                    if bool(_pos_m1.is_short) != bool(is_short_signal):
                        continue  # açık pozisyon yönü ile yeni sinyal aynı değil

                    if is_short_signal:
                        _cur_pnl_pct = (
                            (_pos_m1.entry_price - price) / _pos_m1.entry_price
                            if _pos_m1.entry_price > 0 else 0.0
                        )
                    else:
                        _cur_pnl_pct = (
                            (price - _pos_m1.entry_price) / _pos_m1.entry_price
                            if _pos_m1.entry_price > 0 else 0.0
                        )
                    _tq_for_pyr = _tq_dn_now if is_short_signal else _tq_up_now
                    if (_cur_pnl_pct < M1_SIG_PYR_MIN_PROFIT
                            or _adx_now < M1_SIG_PYR_ADX
                            or _tq_for_pyr < M1_SIG_PYR_TQ):
                        continue
                    if (not is_short_signal) and not (coin_own_bull and _btc_m1_active):
                        continue  # LONG ekleme yalnız coin+BTC yukarı trendde
                    if is_short_signal and (coin_own_bull or in_global_bull):
                        continue  # SHORT ekleme yalnız risk-on dışı, coin ayı trendinde

                    _orig_size = _pos_m1.size_at_entry if _pos_m1.size_at_entry > 0 else _pos_m1.size
                    _add_mult = M1_SIG_PYR_SHORT_SIZE if is_short_signal else M1_SIG_PYR_SIZE
                    _add_size = _orig_size * _add_mult
                    if _add_size <= 0:
                        continue

                    if is_short_signal:
                        _fill = price * (1 - SLIPPAGE)
                        _comm = _fill * _add_size * COMMISSION
                        _add_cost = _fill * _add_size + _comm
                        _coin_allocated = _pos_m1.cost
                        _alloc_cap = initial_capital * M1_SIG_PYR_MAX_ALLOC
                        if (_add_cost < MIN_ORDER_SIZE
                                or _add_cost > balance * M1_SIG_PYR_CASH_CAP
                                or _coin_allocated + _add_cost > _alloc_cap):
                            continue
                        _old_size = _pos_m1.size
                        _new_size = _old_size + _add_size
                        _pos_m1.entry_price = (
                            (_pos_m1.entry_price * _old_size + _fill * _add_size)
                            / _new_size
                        )
                        _pos_m1.size = _new_size
                        _pos_m1.cost += _add_cost
                        balance -= _add_cost
                        _pos_m1.pyramid_count += 1
                        _new_stop = price + 2.0 * atr
                        if _new_stop < _pos_m1.stop_price:
                            _pos_m1.stop_price = _new_stop
                        _new_trail = price + _pos_m1.trailing_mult * atr
                        if _new_trail < _pos_m1.trail_price:
                            _pos_m1.trail_price = _new_trail
                    else:
                        _fill = price * (1 + SLIPPAGE)
                        _comm = _fill * _add_size * COMMISSION
                        _add_cost = _fill * _add_size + _comm
                        _coin_allocated = _pos_m1.cost + getattr(_pos_m1, 'pyramid_cost', 0.0)
                        _alloc_cap = initial_capital * M1_SIG_PYR_MAX_ALLOC
                        if (_add_cost < MIN_ORDER_SIZE
                                or _add_cost > balance * M1_SIG_PYR_CASH_CAP
                                or _coin_allocated + _add_cost > _alloc_cap):
                            continue
                        _pos_m1.size += _add_size
                        _pos_m1.pyramid_cost += _add_cost
                        balance -= _add_cost
                        _pos_m1.pyramid_count += 1
                        _new_stop = price - 2.0 * atr
                        if _new_stop > _pos_m1.stop_price:
                ...
                # ── M5 Signal Pyramid — mevcut LONG pozisyona BUY sinyalinde ek lot ekle ──
                if _m5_sig_pyr:
                    if signal.side != Side.BUY:
                        continue  # BUY değilse ekleme yapma
                    _pos = open_positions[sym]
                    _orig_size = _pos.size_at_entry if _pos.size_at_entry > 0 else _pos.size
                    _add_size  = _orig_size * M5_SIG_PYR_SIZE
                    _fill      = price * (1 + SLIPPAGE)
                    _comm      = _fill * _add_size * COMMISSION
                    _add_cost  = _fill * _add_size + _comm
                    if _add_cost < MIN_ORDER_SIZE or _add_cost > balance * 0.90:
                        continue
                    balance -= _add_cost
                    _pos.size          += _add_size
                    _pos.pyramid_cost   = getattr(_pos, 'pyramid_cost', 0.0) + _add_cost
                    _pos.pyramid_count  = getattr(_pos, 'pyramid_count', 0) + 1
                    # Trailing stop'u mevcut barın ATR'ına göre yeniden ayarla
                    _atr_now   = float(row.get("atr", _pos.entry_atr))
                    _new_trail = price - _pos.trailing_mult * _atr_now
                    if _new_trail > _pos.trail_price:
                        _pos.trail_price = _new_trail
                    continue  # yeni pos açma, pozisyon güncellendi

                # ── SHORT Signal Pyramid execution ───────────────────────────────
                # Açık SHORT'a yeni SHORT sinyali gelince ek lot (LONG M5_SIG_PYR aynası)
                if _short_sig_pyr:
                    if signal.side != Side.SHORT:
                        continue  # SHORT değilse ekleme yapma
                    _spos = open_positions[sym]
                    _s_orig = _spos.size_at_entry if _spos.size_at_entry > 0 else _spos.size
                    _s_add_size = _s_orig * SHORT_PYR_SIZE
                    _s_fill     = price * (1 - SLIPPAGE)     # SHORT: düşük fiyattan sat
                    _s_comm     = _s_fill * _s_add_size * COMMISSION
                    _s_add_cost = _s_fill * _s_add_size + _s_comm
                    if _s_add_cost < MIN_ORDER_SIZE or _s_add_cost > balance * 0.10:
                        continue
                    balance -= _s_add_cost
                    _spos.size          += _s_add_size
                    _spos.pyramid_cost   = getattr(_spos, 'pyramid_cost', 0.0) + _s_add_cost
                    _spos.pyramid_count  = getattr(_spos, 'pyramid_count', 0) + 1
                    # SHORT trailing stop'u aşağı çek (kârı koru)
                    _s_atr_now   = float(row.get("atr", _spos.entry_atr))
                    _s_new_trail = price + _spos.trailing_mult * _s_atr_now
                    if _s_new_trail < _spos.trail_price:
                        _spos.trail_price = _s_new_trail
                    continue  # yeni pos açma, pozisyon güncellendi
    ...
    for sym, pos in list(open_positions.items()):
        ...
        closed_trades.append(pos)
        correlation_registry.register_close(sym)
        # Adaptif profil güncelle (BE - backtest sonu çıkışı)
        if sym in adaptive_profiles and pos.cost > 0:
            _dur_h_be = (pos.exit_time - pos.entry_time).total_seconds() / 3600 if pos.exit_time and pos.entry_time else 0.0
            adaptive_profiles[sym].record_trade(
                pnl         = pos.pnl,
                pnl_pct     = pos.pnl / pos.cost,
                exit_reason = "backtest_end",
                duration_h  = _dur_h_be,
            )
    ...
    if json_out:
        ...
        # Kapalı işlemler: dashboard için son 100, diagnostik için tüm liste.
        def _trade_to_state(_t: PPos) -> dict:
            # v14: closed trade'lere size + cost eklendi, tarihler dakika cinsi
            _t_cost = round(_t.entry_price * _t.size, 2) if _t.size else 0.0
            _hi = _t.max_price_seen or max(_t.entry_price, _t.exit_price)
            _lo = _t.min_price_seen or min(_t.entry_price, _t.exit_price)
            if _t.is_short:
                _real_pct = (_t.entry_price - _t.exit_price) / _t.entry_price * 100 if _t.entry_price else 0.0
                _mfe_pct = (_t.entry_price - _lo) / _t.entry_price * 100 if _t.entry_price else 0.0
                _mae_pct = (_t.entry_price - _hi) / _t.entry_price * 100 if _t.entry_price else 0.0
            else:
                _real_pct = (_t.exit_price - _t.entry_price) / _t.entry_price * 100 if _t.entry_price else 0.0
                _mfe_pct = (_hi - _t.entry_price) / _t.entry_price * 100 if _t.entry_price else 0.0
                _mae_pct = (_lo - _t.entry_price) / _t.entry_price * 100 if _t.entry_price else 0.0
            return {
                "symbol": _t.symbol,
                ...
                "cost": _t_cost,
                "pnl": round(_t.pnl, 2),
                "pnl_pct": round(_real_pct, 2),
                "mfe_pct": round(_mfe_pct, 2),
                "mae_pct": round(_mae_pct, 2),
                "giveback_pct": round(max(0.0, _mfe_pct - _real_pct), 2),
                "exit_reason": _t.exit_reason,
                "bars_held": _t.bars_held,
            }
        _closed_all = [_trade_to_state(_t) for _t in _finished]
        _closed = list(reversed(_closed_all[-100:]))
...
def _print_report(
    ...
    # Coin ortalamalarını önceden hesapla (SONUÇLAR bölümünde göstermek için)
    _traded_bnh  = [s["bnh_pct"] for s in coin_stats if s["n_trades"] > 0]
    _all_bnh     = [s["bnh_pct"] for s in coin_stats if s["bnh_start"] > 0]
    _avg_traded  = float(np.mean(_traded_bnh)) if _traded_bnh else 0.0
    _avg_all     = float(np.mean(_all_bnh))    if _all_bnh    else 0.0
    ...
    # Coin ortalama değişim özeti
    if _traded_bnh:
        _ti = "▲" if _avg_traded >= 0 else "▼"
        _ai = "▲" if _avg_all    >= 0 else "▼"
        print(f"\n  Coin Ort. Değişim:")
        print(f"    İşlem yapılan {len(_traded_bnh)} coin  : {_ti} {_avg_traded:+.1f}%  (piyasa hareketi)")
        print(f"    Tüm {len(_all_bnh)} coin ort.        : {_ai} {_avg_all:+.1f}%  (portföy evreni)")
        _beat = total_pct - _avg_traded
        _beat_icon = "✅ Piyasayı yendi" if _beat > 0 else "❌ Piyasanın altında"
        print(f"    Bot farkı             : {_beat:+.1f} puan  →  {_beat_icon}")
    ...
    # ════════════════════════════════════════════════════════════════════════
    # 3. HATA ANALİZİ — yön, çıkış sebebi, MFE/MAE/giveback
    # ════════════════════════════════════════════════════════════════════════
    if trades:
        def _trade_pct(t: PPos) -> float:
        ...
        def _mfe_mae_giveback(t: PPos) -> tuple[float, float, float]:
            if not t.entry_price:
                return 0.0, 0.0, 0.0
            hi = t.max_price_seen or max(t.entry_price, t.exit_price)
            lo = t.min_price_seen or min(t.entry_price, t.exit_price)
            if t.is_short:
                mfe = (t.entry_price - lo) / t.entry_price * 100
                mae = (t.entry_price - hi) / t.entry_price * 100
            else:
                mfe = (hi - t.entry_price) / t.entry_price * 100
                mae = (lo - t.entry_price) / t.entry_price * 100
            realized = _trade_pct(t)
            return mfe, mae, max(0.0, mfe - realized)

        print(f"\n{'─'*60}")
        print(f"  HATA ANALİZİ")
        print(f"{'─'*60}")
        for label, subset in (
            ("LONG ", [t for t in trades if not t.is_short]),
            ("SHORT", [t for t in trades if t.is_short]),
        ):
            if not subset:
                continue
            pnl = sum(t.pnl for t in subset)
            wr_s = sum(1 for t in subset if t.pnl > 0) / len(subset) * 100
            mfe_avg = float(np.mean([_mfe_mae_giveback(t)[0] for t in subset]))
            gb_avg = float(np.mean([_mfe_mae_giveback(t)[2] for t in subset]))
            print(f"  {label}: {len(subset):>3} işlem | PnL ${pnl:+,.2f} | WR %{wr_s:.1f} | "
                  f"Ort. MFE %{mfe_avg:.2f} | Geri verme %{gb_avg:.2f}")

        by_reason: dict[str, list[PPos]] = {}
        for t in trades:
            by_reason.setdefault(t.exit_reason or "?", []).append(t)
        parts = []
        for reason, subset in sorted(by_reason.items(), key=lambda kv: sum(t.pnl for t in kv[1])):
            pnl = sum(t.pnl for t in subset)
            gb_avg = float(np.mean([_mfe_mae_giveback(t)[2] for t in subset]))
            parts.append(f"{reason}:{len(subset)} işlem/${pnl:+.0f}/GB%{gb_avg:.1f}")
        print(f"  Çıkış sebebi    : {' | '.join(parts)}")
    ...
    for i, t in enumerate(sorted_trades, 1):
        ...
        side_tag = "⬇SHORT" if getattr(t, "is_short", False) else "⬆LONG "
        print(f"  {i:>3}  {t.symbol:<11} {e_str:<12} {dur:>6}  "
              f"{t.entry_price:>10.4f} {t.exit_price:>10.4f}  "
              f"{icon}${abs(t.pnl):>6.2f}  {reason_short}  {side_tag}")
...
def main() -> None:
    ...
    parser.add_argument("--timeframe", "--tf", dest="timeframe", type=str, default="1h",
                        help="Bar timeframe: 1h (varsayılan), 15m (--m5 modunda otomatik), 5m, 1m")
    ...
    # Timeframe: --timeframe argümanı (varsayılan 1h)
    # M4/M5 → 15m, M6/M7 → 1m (mevcut TF mantığı korundu)
    # --timeframe 15m için en az --m5 modunu da aktif et (parametreler 15m kalibreli)
    _tf_arg = getattr(args, "timeframe", "1h") if not (m4_mode or m5_mode or m6_mode or m7_mode or m8_mode) else None
    if _tf_arg == "15m" and not m5_mode:
        # 15m universe modunda M5 filtrelerini aktif et (noise azaltma)
        m5_mode = True
        m4_mode = True
        auto_mode = True
        use_wfo = False
    ...
    if args.start_date:
        try:
            ...
            # v37: 15m EMA800 warmup = 960 bar × 15min = 10 gün. 14 gün medium_momentum ek buffer.
            # 1h EMA200 warmup = 210 bar × 60min = 8.75 gün.
            _tf_for_warmup = _tf_arg or "15m"
            if _tf_for_warmup == "15m":
                _warmup_days = 25  # EMA800(10g) + medium_momentum(14g) + 1g buffer
            elif _tf_for_warmup == "1h":
                _warmup_days = WARMUP_BARS // 24 + 2
            else:
                _warmup_days = 10
            wfo_extra = WFO_LOOKBACK if (args.wfo or args.auto or args.m4) else 0
            days_needed = (datetime.now() - start_dt).days + _warmup_days + wfo_extra + 10
            if args.days != 365:
    ...
    run_portfolio_backtest(
        ...
        use_universe=True,  # Varsayılan: her zaman 26-coin universe modu
        ...
        m7_mode=m7_mode,
        m8_mode=m8_mode,
        timeframe=_tf_arg,   # 15m varsayılan (mod olmadığında)
    )
